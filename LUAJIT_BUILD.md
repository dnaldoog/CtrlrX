...
The LuaJIT source is distributed as a zip file included in the repository at:

```
Source/Resources/luajit.zip
```

Before building, extract it to the correct location:
...
This extracts the `luajit` folder to `Source/Misc/luajit/` which is where CMake expects it. The zip includes:
- All LuaJIT source files
- Pre-configured `CMakeLists.txt`
- Pre-configured `.gitignore` for build artifacts
- Pre-built `lua51.lib` and `luajit.h` for Windows (no need to run `msvcbuild.bat` unless upgrading LuaJIT)

After extracting, proceed to the build steps for your platform below.
...
Only needed if upgrading LuaJIT to a newer version — the pre-built `lua51.lib` and `luajit.h` in the zip are ready to use for normal builds.
...
> **Note for subsequent clones:**  
> Once `lua51.lib` and `luajit.h` are committed to the repo, anyone cloning the  
> repository does **not** need to run `msvcbuild.bat` — the files are already there.  
> Only run `msvcbuild.bat` again if upgrading LuaJIT to a newer version.
...
- If `Source\Misc\luajit\src\msvcbuild.bat` is found and `lua51.lib` is missing → LuaJIT is built automatically
- If `lua51.lib` already exists → build is skipped, existing lib is used
- If the luajit folder is absent (non-LuaJIT branch) → LuaJIT is silently skipped, standard Lua is used
- `-DCTRLRX_USE_LUAJIT=ON` is passed to CMake automatically when LuaJIT source is present
...
After rebuilding `lua51.lib` or `libluajit.a`, always commit both the library and the header:

```cmd
git add Source\Misc\luajit\src\lua51.lib
git add Source\Misc\luajit\src\luajit.h
git commit -m "Rebuild LuaJIT"
```

Then do a full clean build to ensure CMake picks up the new library:
