...
class CtrlrFontManager {
	public:
		enum FontSet { osFontSet, importedFontSet, builtInFontSet, juceFontSet, unknownFontSet };
		...
		void fillCombo(ComboBox &comboToFill, const bool showOsFonts = true, const bool showBuiltInFonts = true,
					   const bool showImportedFonts = true, const bool showJuceFonts = true);
		...
		static bool isFontFile(const File &fontFile) {
			return (fontFile.hasFileExtension("jfont") || fontFile.hasFileExtension("ttf") ||
					fontFile.hasFileExtension("otf"));
		}
