...
class CtrlrManager : public ValueTree::Listener,
	...
	public:
		...
		enum CtrlrModulatorMapType { ModulatorByName, ModulatorByMidiControllerNumber, ModulatorBySysexFormula };
		...
		*/
		...
		/** Add a new Panel
			...
			@param showUI		if set to TRUE the UI of this panel will be shown, otherwise the panel will only be
		   created
		...
		*/
		...
		const var getProperty(const Identifier &name, const var &defaultReturnValue) const {
			return managerTree.getProperty(name, defaultReturnValue);
		}
