...
String CtrlrPanelEvaluationScope::getScopeUID() const { return ("panel"); }
...
String CtrlrGlobalEvaluationScope::getScopeUID() const { return ("global"); }

Expression CtrlrGlobalEvaluationScope::getSymbolValue(const String &symbol) const {
	if ((symbol.startsWith("k") || symbol.startsWith("l") || symbol.startsWith("m") || symbol.startsWith("n")) &&
		symbol.length() >= 2) {
...
double evaluateFormulaFunction(CtrlrPanel &panel, const String &functionName, const double *parameters,
							   int numParameters) {
