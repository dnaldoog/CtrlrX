...
CtrlrPanelResource::CtrlrPanelResource(CtrlrPanelResourceManager &_owner, const File &_resourceDataFile,
	...
	resourceHashCode = computeContentHash(_resourceDataFile);
	...
	setProperty(Ids::resourceHash, resourceHashCode, false);
...
File &CtrlrPanelResource::getFile() { return (resourceDataFile); }

int64 CtrlrPanelResource::getSize() { return (resourceDataFile.getSize()); }

double CtrlrPanelResource::getSizeDouble() { return ((double)getSize()); }

int64 CtrlrPanelResource::getHashCode() { return (getProperty(Ids::resourceHash)); }

const String CtrlrPanelResource::getName() { return (resourceName); }
...
const String CtrlrPanelResource::asText() { return (resourceDataFile.loadFileAsString()); }
...
const XmlElement *CtrlrPanelResource::asXml() { return (XmlDocument::parse(resourceDataFile).release()); }
...
AudioFormatReader *CtrlrPanelResource::asAudioFormat() {
	...
	audioFormatReader =
		owner.getOwner().getCtrlrManagerOwner().getAudioFormatManager().createReaderFor(resourceDataFile);
...
bool CtrlrPanelResource::isLoaded() { return (resourceLoaded); }
...
void CtrlrPanelResource::calculateHash() {
	setProperty(Ids::resourceHash,
				resourceDataFile.hashCode64() + resourceDataFile.getLastModificationTime().toMilliseconds());
}
...
void CtrlrPanelResource::load() {
	...
	} else if (resourceType == AudioRes || resourceType == XmlRes || resourceType == TextRes ||
...
Time CtrlrPanelResource::getLoadedTime() { return (Time((int64)getProperty(Ids::resourceLoadedTime))); }
...
CtrlrPanelResourceType CtrlrPanelResource::getType() { return (resourceType); }
...
ValueTree &CtrlrPanelResource::getResourceTree() { return (resourceTree); }
...
InputStream *CtrlrPanelResource::createInputStream() { return resourceDataFile.createInputStream().release(); }
// added for gzip support
