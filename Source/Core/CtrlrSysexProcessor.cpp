...
int CtrlrSysexProcessor::getGlobalIndex(const CtrlrSysexToken token) { return (token.getAdditionalData()); }

void CtrlrSysexProcessor::sysExProcessToken(const CtrlrSysexToken token, uint8 *byte, const int value,
											const int channel) {
	if (byte == NULL)
		return;
	if (token.getType() >= Nibble16bitLsb0 && token.getType() <= Nibble16bitMsb3) {
		_DBG("Processing token type=" + String(token.getType()) + " position=" + String(token.getPosition()) +
			 " value=" + String(value) + " channel=" + String(channel));
	}
	BigInteger bi;
...
void CtrlrSysexProcessor::sysExProcess(const Array<CtrlrSysexToken> &tokens, MidiMessage &m, const int value,
									   const int channel) {
...
void CtrlrSysexProcessor::sysexProcessPrograms(const Array<CtrlrSysexToken> &tokens, MidiMessage &m) { return; }

CtrlrMidiMessageEx CtrlrSysexProcessor::sysexMessageFromString(const String &formula, const int value,
															   const int channel) {
	MidiMessage m;
...
double CtrlrSysexProcessor::getValueFromSysExData(const Array<CtrlrSysexToken> &tokens,
												  const CtrlrMidiMessageEx &message) {
	double v = 0;
...
Array<CtrlrSysexToken> CtrlrSysexProcessor::sysExToTokenArray(const String &formula) {
	...
	for (int i = 0; i < ar.size(); i++) {
		...
		if (tokenToAdd.getType() == GlobalVariable || tokenToAdd.getType() == ChecksumRolandJP8080 ||
			tokenToAdd.getType() == ChecksumTechnics // Added v5.6.34.
			|| tokenToAdd.getType() == ChecksumOnesComplement || tokenToAdd.getType() == ChecksumSummingSimple ||
			...
			*/
			tokenToAdd.setAdditionalData(
				ar[i].substring(1).trim().getIntValue()); // Updated v5.6.34. Thanks to @dnaldoog
		}
...
void CtrlrSysexProcessor::checksumRolandJp8080(const CtrlrSysexToken token,
											   MidiMessage &m) // Update v5.6.34. Thanks to @dnaldoog
{
	_DBG("I am Roland");
...
void CtrlrSysexProcessor::checksumSummingSimple(const CtrlrSysexToken token, MidiMessage &m) {
	...
	for (int i = startByte; i < token.getPosition(); i++) {
		chTotal =
			chTotal + *(ptr + i); // Some implementations just add up the data, but still need to mask it to 7 bits
	}
...
void CtrlrSysexProcessor::showMidiHelp() {
	...
	if (auto *okButton = alert->getButton("OK")) {
		const int bW = 80;
		const int bH = 40;
		okButton->setBounds((alert->getWidth() - bW) / 2, alert->getHeight() - bH - 30, bW, bH);
	}
