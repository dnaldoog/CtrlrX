...
static const char *asciiSpecials[] = {"[NUL]", "[SOH]", "[STX]", "[ETX]", "[EOT]", "[ENQ]", "[ACK]", "[BEL]", "[BS]",
									  "[HT]",  "[LF]",	"[VT]",	 "[FF]",  "[CR]",  "[SO]",	"[SI]",	 "[DLE]", "[DC1]",
									  "[DC2]", "[DC3]", "[DC4]", "[NAK]", "[SYN]", "[ETB]", "[CAN]", "[EM]",  "[SUB]",
									  "[ESC]", "[FS]",	"[GS]",	 "[RS]",  "[US]",  0};
static const uint8 vendors_id[] = {1,	 2,	   3,	 4,	   5,	 6,	   7,	 8,	   9,	 0x0A, 0x0B, 0x0C, 0x0D, 0x0E,
								   0x10, 0x11, 0x12, 0x13, 0x14, 0x1B, 0x15, 0x16, 0x17, 0x18, 0x20, 0x21, 0x23, 0x24,
								   0x25, 0x26, 0x27, 0x28, 0x29, 0x2F, 0x36, 0x3E, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45};
	...
	if (tokens.size() >= 3) {
		...
		if (tokens[2] == "CCCoarseMSB") {
			...
			if (number > 31) {
				...
				_WRN("CCFineLSB: coarse CC number " + String(number) +
					 " is out of range (must be 0-31). Fine message suppressed.");
				return CtrlrMidiMessageEx(); // return empty, caller should discard
			}
			finalNumber = number + 32;
			finalValue = (value & 1) << 6;
			_DBG("CCFineLSB: coarse=" + String(number) + " fine=" + String(finalNumber) + " input=" + String(value) +
...
const String generateRandomUnique(const String &additionalRandomData) {
	...
	String randData = additionalRandomData + SystemStats::getComputerName() + SystemStats::getCpuVendor() +
					  SystemStats::getFullUserName() + SystemStats::getJUCEVersion() + SystemStats::getLogonName() +
...
const String getRawDataSize(const MidiMessage &m) { return (String::formatted(" Size:[%6d]", m.getRawDataSize())); }
...
const String getTimestamp(const MidiMessage &m) { return (String::formatted(" Time(%.6f)", m.getTimeStamp())); }

const String getTimestamp(const double t) { return (String::formatted(" Time(%.6d)", t)); }
...
const String getMidiMessageAsLogString(const MidiMessage &m, const double customTimestamp, const bool name,
									   const bool channel, const bool number, const bool value, const bool timestamp,
									   const bool rawData, const bool rawInDecimal, const bool rawDataSize) {
	return ((timestamp ? (customTimestamp >= 0 ? getTimestamp(customTimestamp) : getTimestamp(m)) : "") +
			(rawDataSize ? getRawDataSize(m) : "") + (name ? getName(m) : "") + (channel ? getMidiChannel(m) : "") +
			(number ? getMidiNumber(m) : "") + (value ? getMidiValue(m) : "") +
...
const String dataPrefix(const MemoryBlock &data, const int prefixLength) {
	return ("(" + STR((int)data.getSize()) + ")" +
			String::toHexString(data.getData(),
								((prefixLength < (int)data.getSize()) ? prefixLength : (int)data.getSize())));
}
...
const MemoryBlock midiMessagePattern(const CtrlrMidiMessageEx &mEx, const Array<CtrlrSysexToken> tokens,
									 const Array<int, CriticalSection> &globalVariables) {
	if (mEx.m.isNoteOff() || mEx.m.isNoteOn()) {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			} else if (tokens[i].getType() != NoToken || tokens[i].getType() == Ignore ||
					   tokens[i].getType() == ChecksumXor || tokens[i].getType() == ChecksumRolandJP8080 ||
					   tokens[i].getType() == ChecksumOnesComplement || tokens[i].getType() == ChecksumSummingSimple ||
...
void channelizeBuffer(MidiBuffer &inputBuffer, MidiBuffer &outputBuffer, const int channel,
					  const bool channelizeAllowed) {
...
void restoreProperties(const ValueTree &sourceTree, ValueTree &destinationTree, UndoManager *undoManager,
					   const String &propertyPrefix) {
	if (propertyPrefix == "") {
		for (int i = sourceTree.getNumProperties(); --i >= 0;) {
			destinationTree.setProperty(sourceTree.getPropertyName(i),
										sourceTree.getProperty(sourceTree.getPropertyName(i)), undoManager);
		}
