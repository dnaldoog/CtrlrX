  ...
  public:
    ...
    static void crashHandler() {
        if (JUCEApplication::isStandaloneApp()) {
            MemoryBlock mb(SystemStats::getStackBacktrace().toUTF8(),
                           SystemStats::getStackBacktrace().length());
            ...
            File crashFile(
                File::getSpecialLocation(File::currentApplicationFile).getFileExtension() +
                ".crash");
    ...
    void initialise(const String& commandLineParameters) {
...

        {
            ...
            if (!commandLineParameters.isEmpty()) {
                ...
                if (parameters.contains("crashReport")) {
                    ...
                    File crashReportFile(crashReportForExec.withFileExtension(
                        crashReportForExec.getFileExtension() + ".crash"));
                    AlertWindow crashReport(
                        "Ctrlr has crashed",
                    ...
                    if (parameters.contains("stackTrace")) {
                        if (!parameterValues[parameters.indexOf("stackTrace")].isEmpty()) {
                            ...
                            mb.fromBase64Encoding(
                                parameterValues[parameters.indexOf("stackTrace")]);
        ...
        filterWindow = new CtrlrStandaloneWindow(ProjectInfo::projectName + String("/") +
