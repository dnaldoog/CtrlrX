...
CtrlrStandaloneWindow::CtrlrStandaloneWindow(const String& title, const Colour& backgroundColour)
    : DocumentWindow(title, backgroundColour, DocumentWindow::allButtons, true),
      ctrlrProcessor(nullptr), filter(nullptr), appProperties(nullptr), restoreState(true) {
    ...
    centreWithSize(
        800, 600); // set Size of the whole app with title bar included H22px and borders 2x1px
    ...
    if (filter != 0) {
        ...
        if (appProperties != nullptr) {
            ...
            ScopedPointer<XmlElement> xml(appProperties->getUserSettings()
                                              ->getXmlValue(CTRLR_PROPERTIES_FILTER_STATE)
                                              .release());
            ...
            if (appProperties->getUserSettings()->getValue(CTRLR_PROPERTIES_WINDOW_STATE, "") !=
                ...
                restoreWindowStateFromString(
                    appProperties->getUserSettings()->getValue(CTRLR_PROPERTIES_WINDOW_STATE));
            } else {
                _DBG("CTRLR_PROPERTIES_WINDOW_STATE == null");
                if (ctrlrProcessor->getManager()
                        .getInstanceTree()
                        .getChildWithName(Ids::uiPanelEditor)
                        .isValid()) {
                    _DBG("uiPanelEditor isValid");
                    ValueTree ed = ctrlrProcessor->getManager().getInstanceTree().getChildWithName(
                        Ids::uiPanelEditor);
                    Rectangle<int> r =
                        VAR2RECT(ed.getProperty(Ids::uiPanelCanvasRectangle, "0 0 800 600"));
                    int menuBarHeight =
                        (int)ctrlrProcessor->getManager().getProperty(Ids::ctrlrMenuBarHeight);
                    if (menuBarHeight <= 0)
                        menuBarHeight = 24;
                    ...
                    centreWithSize(r.getWidth() <= 0 ? 800 : r.getWidth(),
                                   (r.getHeight() <= 0 ? 600 : r.getHeight()) +
                                       (menuBarVisible ? menuBarHeight : 0));
    ...
    ValueTree ed =
        ctrlrProcessor->getManager().getInstanceTree().getChildWithName(Ids::uiPanelEditor);
    ...
    vpOsFrameBtm =
        getPeer()->getFrameSize().getBottom();           // OS Native Window Border Bottom thickness
    vpOsFrameLeft = getPeer()->getFrameSize().getLeft(); // OS Native Window Border Left thickness
    vpOsFrameRight =
        getPeer()->getFrameSize().getRight();            // OS Native Window Border Right thickness
        ...
        if (auto* constrainer = getConstrainer()) {
            if (vpEnableFixedAspectRatio == true) {
                constrainer->setFixedAspectRatio(
                    vpStandaloneAspectRatio); // set window aspect ratio

                if (vpEnableResizableLimits == true) {
                    if (vpMinWidth != 0 && vpMaxWidth != 0) {
                        setResizeLimits(vpMinWidth, round(vpMinWidth / vpStandaloneAspectRatio),
                                        vpMaxWidth, round(vpMaxWidth / vpStandaloneAspectRatio));
                        ...
                        constrainer->setMinimumSize(
                            panelCanvasWidth, panelCanvasHeight + vpOsFrameTop + vpOsFrameBtm);
                    }
                }
...
void CtrlrStandaloneWindow::changeListenerCallback(
    ChangeBroadcaster* source) { // Check for window title modification
...
void CtrlrStandaloneWindow::saveStateNow() {
    ...
    if (ctrlrProcessor != nullptr && appProperties != nullptr) {
        appProperties->getUserSettings()->setValue(CTRLR_PROPERTIES_WINDOW_STATE,
        ...
        if (data.getSize() > 0) {
            ScopedPointer<XmlElement> xml(
                CtrlrProcessor::getXmlFromBinary(data.getData(), (int)data.getSize()));
