...
// #include <luabind/call_function.hpp>
...
void LAlertWindow::queryText(AlertIconType iconType, const String &title, const String &textMessage,
							 const String &textAreaContent, const String &textAreaLabel, const String &button1Text,
							 const String &button2Text, bool isContentPassword, luabind::object const &result) {
	...
	enterModalState(true, ModalCallbackFunction::create([safeThis](int ret) {
						if (safeThis != nullptr && safeThis->o.is_valid()) {
							try {
								...
								// CtrlrLog::formatMessage(CtrlrLog::CtrlrLogMessage(juce::String("Error in AlertWindow
								// modal handler callback: ") + e.what()));
							}
						}
					}),
					false);
...
void LComponent::wrapForLua (lua_State *L)
{
	...
	module(
		L)[class_<DropShadow>("DropShadow")
			   .def(constructor<>())
		   ...
		   class_<Component, LComponent>("Component")
			   ...
			   .def("getLocalPoint",
					(Point<int> (Component::*)(const Component *, Point<int>) const) & Component::getLocalPoint)
			   ...
			   .def("setTopRightPosition", (void (Component::*)(int, int))&Component::setTopRightPosition)
			   .def("setTopRightPosition", (void (Component::*)(Point<int>))&Component::setTopRightPosition)
			   ...
			   .def(
				   "reallyContains",
				   +[](Component *self, Point<int> pt,
					   bool returnTrueIfWithinAChild) { return self->reallyContains(pt, returnTrueIfWithinAChild); })
			   ...
			   .def(
				   ...
				   +[](Component *self) -> ComponentTraverser
											* {
												// .release() transfers ownership of the unique_ptr out to a raw pointer
												return self->createFocusTraverser().release();
											})
			   ...
			   .scope[def("getCurrentlyFocusedComponent", &LComponent::getCurrentlyFocusedComponent),
...
std::unique_ptr<DropShadower> LookAndFeelBase::createDropShadowerForComponent(Component& component)
{
    ...
    if (methods.contains("createDropShadowerForComponent"))
    {
        try
        {
            // Pass &component (pointer) or component (ref) depending on what Lua expects,
            // but passing &component is typical for Luabind object pointers.
            ...
            if (s != nullptr)
            {
                // Wrap raw pointer into unique_ptr to match return signature
...
static void LPopupMenu_showAsync(PopupMenu& menu, luabind::object luaCallback)
{
    auto options = juce::PopupMenu::Options();
    
    menu.showMenuAsync(options, [luaCallback](int result) mutable
    {
        if (result != 0 && luaCallback.is_valid())
        {
            juce::MessageManager::callAsync([luaCallback, result]() mutable
            {
                try
                {
                    luabind::call_function<void>(luaCallback, result);
                }
                catch (const std::exception& e)
                {
...
// Full coordinate showAsync matching show(0,0,0,0, callback)
static void LPopupMenu_showAsyncWithRect(PopupMenu& menu, 
...
void LTextEditor::wrapForLua (lua_State *L)
{
	...
	module(L)[class_<TextEditor, bases<Component>>("TextEditor")
				  .def(constructor<>())
				  .def(constructor<const String &>())
				  ...
				  .def(
					  "getTextIndexAt", +[](const TextEditor *self, Point<int> pt) { return self->getTextIndexAt(pt); })
