...
{
#if JUCE_VERSION >= 0x070000
    // --- Modern JUCE 7/8 Asynchronous Native Dialog ---
    // Note: Because it is asynchronous, this function returns instantly.
    // If you need the result from it, you must pass a callback lambda to showAsync.
    MessageBoxOptions options = MessageBoxOptions::makeOptionsOkCancel (
    ...
    return true;
#else
    // --- Legacy JUCE 6 Synchronous Native Dialog ---
...
// --- Instance-based Asynchronous Handling Replacement ---
void LAlertWindow::runModalLoopAsync()
{
#if JUCE_VERSION >= 0x070000
    // Use Component::SafePointer to intercept dangling object scenarios if the underlying pane drops mid-flight
    ...
    enterModalState(true, ModalCallbackFunction::create([safeThis](int ret) {
        ...
        {
            try {
                luabind::call_function<void>(safeThis->o, ret, safeThis.getComponent());
            } catch (const std::exception& e) {
                // Ensure Lua execution failures don't drop down to the desktop layer
                CtrlrLog::formatMessage("Error in AlertWindow modal handler callback: " + String(e.what()));
            }
        }
    }), false); // False here keeps instance allocation constraints bound to the calling Lua script lifecycle scope
#else
