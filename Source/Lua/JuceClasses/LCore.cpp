...
void LExpression::wrapForLua(lua_State *L) {
	...
	module(L)[class_<Expression::Scope>("Scope"), class_<Expression::Symbol>("Symbol"),
			  class_<Expression>("Expression")
				  ...
				  .def("evaluate",
					   (double (Expression::*)(const Expression::Scope &, String &) const) & Expression::evaluate)
...
void LFile::findChildFiles(const File &file, luabind::object const &table, int whatToLookFor, bool searchRecursively,
...
double LFile::getSize(const File &file) { return ((double)file.getSize()); }
...
bool LFile::isValid(const File &file) { return !(file == File()); }
...
void LFile::wrapForLua(lua_State *L) {
	...
	module(L)
		[class_<File>("File")
			 ...
			 // .def("replaceWithData", &File::replaceWithData) // Removed v5.6.35. Low-level C++ API, difficult to use
			 // from Lua.
			 .def("replaceWithData",
				  &LFile::replaceWithData) // High-level Lua-friendly wrapper that accepts LMemoryBlock objects directly
			 ...
			 .enum_("TypesOfFileToFind")[value("findDirectories", 1), value("findFiles", 2),
										 value("findFilesAndDirectories", 3), value("ignoreHiddenFiles", 4)]
			 .enum_("SpecialLocationType")
				 [value("userHomeDirectory", 0), value("userDocumentsDirectory", 1), value("userDesktopDirectory", 2),
				  value("userMusicDirectory", 3), value("userMoviesDirectory", 4), value("userPicturesDirectory", 5),
				  value("userApplicationDataDirectory", 6), value("commonApplicationDataDirectory", 7),
				  value("commonDocumentsDirectory", 8), value("tempDirectory", 9), value("currentExecutableFile", 10),
				  value("currentApplicationFile", 11), value("invokedExecutableFile", 12),
				  value("hostApplicationPath", 13), value("windowsSystemDirectory", 14),
				  value("globalApplicationsDirectory", 15), value("globalApplicationDirectoryX86", 16),
				  value("windowsLocalAppData", 17)]
			 .scope[def("descriptionOfSizeInBytes", &LFile::descriptionOfSizeInBytes),
					def("findFileSystemRoots", &File::findFileSystemRoots),
					def("getSpecialLocation", &LFile::getSpecialLocation), def("createTempFile", &File::createTempFile),
...
double LGlobalFunctions::double_Pi() { return (juce::double_Pi); }

float LGlobalFunctions::float_Pi() { return (juce::float_Pi); }

void LGlobalFunctions::debug(const String &argument) { _LUA(argument); }
...
const int LGlobalFunctions::int64ToInt(const int64 value) { return (static_cast<int>(value)); }
...
void LGlobalFunctions::wrapForLua(lua_State *L) {
	...
	module(
		L)[def("getNativeKeyMapping", &getNativeKeyMapping),
		   ...
		   def("_debug", (void (*)(const String &))&LGlobalFunctions::debug), def("sleep", &LGlobalFunctions::sleep),
		   class_<LGlobalFunctions>("juce")
			   .def(constructor<>())
			   .scope[def("double_Pi", &LGlobalFunctions::double_Pi), def("float_Pi", &LGlobalFunctions::float_Pi)],
		   class_<CtrlrNative>("CtrlrNative")
			   .def("sendKeyPressEvent",
					(Result (CtrlrNative::*)(const KeyPress &, const String &))&CtrlrNative::sendKeyPressEvent)
			   .def("sendKeyPressEvent", (Result (CtrlrNative::*)(const KeyPress &))&CtrlrNative::sendKeyPressEvent)
			   .scope[def("getNativeObject", &CtrlrNative::getNativeObject)],
		   ...
		   def("isPositiveAndNotGreaterThan",
			   (bool (*)(const double, const double))&juce::isPositiveAndNotGreaterThan<double>),
		   def("roundToInt", &juce::roundToInt<double>), def("roundToIntAccurate", &juce::roundToIntAccurate),
		   def("roundDoubleToInt", &juce::roundDoubleToInt), def("roundFloatToInt", &juce::roundFloatToInt),
		   def("isPowerOfTwo", (bool (*)(int))&juce::isPowerOfTwo<int>), def("nextPowerOfTwo", &juce::nextPowerOfTwo),
		   def("jmap", (double (*)(const double, const double, const double, const double,
								   const double))&juce::jmap<double>), // Added 5.6.34. Thanks to @dnaldoog
		   def("jmap", (double (*)(const double, const double,
								   const double))&juce::jmap<double>), // Added 5.6.34. Thanks to @dnaldoog
		   def("mapToLog10", (double (*)(const double, const double,
										 const double))&juce::mapToLog10<double>), // Added 5.6.34. Thanks to @dnaldoog
		   def("mapFromLog10",
			   (double (*)(const double, const double,
						   const double))&juce::mapFromLog10<double>), // Added 5.6.34. Thanks to @dnaldoog
		   def("isWithin", (double (*)(const double, const double,
									   const double))&juce::isWithin<double>), // Added 5.6.34. Thanks to @dnaldoog

		   def("cos", (double (*)(double))&cos), def("cosf", (float (*)(float))&cosf),
		   def("sin", (double (*)(double))&sin), def("sinf", (float (*)(float))&sinf),
		   def("tan", (double (*)(double))&tan), def("acos", (double (*)(double))&acos),
		   def("asin", (double (*)(double))&asin), def("atan", (double (*)(double))&atan),
		   def("atan2", (double (*)(double, double))&atan2), def("cosh", (double (*)(double))&cosh),
		   def("sinh", (double (*)(double))&sinh), def("tanh", (double (*)(double))&tanh),
		   def("exp", (double (*)(double))&exp), def("frexp", (double (*)(double, int *))&frexp),
		   def("ldexp", (double (*)(double, int))&ldexp), def("log", (double (*)(double))&log),
		   def("log10", (double (*)(double))&log10), def("modf", (double (*)(double, double *))&modf),
		   def("pow", (double (*)(double, double))&pow), def("powf", (float (*)(float, float))&powf),
		   def("sqrt", (double (*)(double))&sqrt), def("ceil", (double (*)(double))&ceil),
		   def("ceilf", (float (*)(float))&ceilf), def("fabs", (double (*)(double))&fabs),
		   def("fabsf", (float (*)(float))&fabsf), def("floor", (double (*)(double))&floor),
		   def("floorf", (float (*)(float))&floorf), def("fmod", (double (*)(double, double))&fmod),
		   def("fmodf", (float (*)(float, float))&fmodf), def("int64ToInt", &int64ToInt),
...
void LKeyPress::wrapForLua(lua_State *L) {
	...
	module(
		L)[class_<KeyPress>("KeyPress")
			   ...
			   .scope[def("createFromDescription", &KeyPress::createFromDescription),
					  def("isKeyCurrentlyDown", &KeyPress::isKeyCurrentlyDown)]
			   .enum_("KeyCode")[value("spaceKey", KeyPress::spaceKey), value("escapeKey", KeyPress::escapeKey),
								 value("returnKey", KeyPress::returnKey), value("tabKey", KeyPress::tabKey),
								 value("deleteKey", KeyPress::deleteKey), value("backspaceKey", KeyPress::backspaceKey),
								 value("insertKey", KeyPress::insertKey), value("upKey", KeyPress::upKey),
								 value("downKey", KeyPress::downKey), value("leftKey", KeyPress::leftKey),
...
void LModifierKeys::wrapForLua(lua_State *L) {
	...
	module(L)[class_<ModifierKeys>("ModifierKeys")
				  .def(constructor<int>())
				  ...
				  .scope[def("getCurrentModifiers", &ModifierKeys::getCurrentModifiers),
						 def("getCurrentModifiersRealtime", &ModifierKeys::getCurrentModifiersRealtime)]
				  .enum_("Flags")[value("shiftModifier", 1), value("ctrlModifier", 2), value("altModifier", 4),
								  value("leftButtonModifier", 16), value("rightButtonModifier", 32),
								  value("middleButtonModifier", 64), value("commandModifier", 8),
								  value("popupMenuClickModifier", 34), value("allKeyboardModifiers", 15),
...
void LMouseCursor::wrapForLua(lua_State *L) {
	...
	module(L)[class_<MouseCursor>("MouseCursor")
				  .def(constructor<>())
				  ...
				  .enum_("StandardCursorType")
					  [value("ParentCursor", MouseCursor::ParentCursor), value("NoCursor", MouseCursor::NoCursor),
					   value("NormalCursor", MouseCursor::NormalCursor), value("WaitCursor", MouseCursor::WaitCursor)]
				  .scope[def("showWaitCursor", &MouseCursor::showWaitCursor),
...
void LMouseEvent::wrapForLua(lua_State *L) {
	...
	module(L)[class_<MouseEvent>("MouseEvent")
...
void LMouseInputSource::wrapForLua(lua_State *L) {
	...
	module(
		L)[class_<MouseInputSource>("MouseInputSource")
			   .def(constructor<const MouseInputSource &>())
			   .enum_("MouseInputSource")[value("mouse", MouseInputSource::mouse),
										  value("touch", MouseInputSource::touch), value("pen", MouseInputSource::pen)]
...
void LRandom::wrapForLua(lua_State *L) {
	...
	module(L)[class_<Random>("Random")
				  ...
				  .scope[def("getSystemRandom", &Random::getSystemRandom)]];
}

void LRange::wrapForLua(lua_State *L) {
	using namespace luabind;

	module(L)[class_<Range<int>>("Range")
				  .def(constructor<>())
...
void LString::wrapForLua(lua_State *L) {
	...
	module(L)[class_<String>("String")
			  ...
			  class_<CharPointer_UTF8>("CharPointer_UTF8").def("getAddress", &CharPointer_UTF8::getAddress),
...
void LTime::wrapForLua(lua_State *L) {
	...
	module(L)[class_<LTime>("Ltime"),
			  class_<Time, bases<LTime>>("Time")
				  ...
				  .scope[def("getCurrentTime", &LTime::getCurrentTime),
...
void LTimer::setCallback(luabind::object _callback) { callback = _callback; }
...
void LTimer::wrapForLua(lua_State *L) {
	...
	module(L)[class_<Timer, LTimer>("Timer")
...
void LURL::wrapForLua(lua_State *L) // Updated v5.6.34. Required for Online Registration
{
	...
	module(L)[class_<URL>("URL")
				  ...
				  // Correct static_cast for readEntireTextStream (assuming the bool overload, which is common for
				  // POST/GET)
				  .def("readEntireTextStream", (String (URL::*)(bool) const) & URL::readEntireTextStream)
				  // If you also use the version that takes email/password/timeout/extraHeaders, you'd need another
				  // .def: .def("readEntireTextStreamComplex", (String (URL::*)(bool, String, String, int, String)
				  // const) &URL::readEntireTextStream)
...
void LValueTree::wrapForLua(lua_State *L) {
	...
	module(
		L)[class_<Identifier>("Identifier")
			   .def(constructor<const char *>())
			   .def(constructor<const String &>())
			   .def("toString", &Identifier::toString)
			   .scope[def("isValidIdentifier", &Identifier::isValidIdentifier)],
		   class_<var>("var")
			   .def(constructor<const var &>())
			   .def(constructor<int>())
			   .def(constructor<int64>())
		   ...
		   class_<ValueTree>("ValueTree")
			   ...
			   .def("addChild",
					(void (ValueTree::*)(const ValueTree &, int, UndoManager *))&ValueTree::addChild) // Added v5.6.34
			   .def("addChild",
					(void (ValueTree::*)(const ValueTree &, UndoManager *))&ValueTree::addChild) // Added v5.6.34
...
void LXmlElement::wrapForLua(lua_State *L) {
	...
	module(
		L)[class_<XmlElement>("XmlElement")
			   .def(constructor<const String &>())
			   ...
			   .def("getStringAttribute",
					(const String &(XmlElement::*)(StringRef) const) & XmlElement::getStringAttribute)
			   .def("getStringAttribute",
					(String (XmlElement::*)(StringRef, const String &) const) & XmlElement::getStringAttribute)
			   // .def("compareAttribute", &XmlElement::compareAttribute)
			   .def("compareAttribute",
					(bool (XmlElement::*)(StringRef, StringRef, bool) const) & XmlElement::compareAttribute)
			   ...
			   .def("getChildElementAllSubText", &XmlElement::getChildElementAllSubText)
			   .scope[def("createTextElement", &XmlElement::createTextElement)],
		   class_<XmlDocument>("XmlDocument")
			   .def(constructor<const String &>())
			   .def(constructor<const File &>())
			   .def("getDocumentElement", &getDocumentElementWrapper,
					adopt(result)) // was commented out?? https://github.com/damiensellier/CtrlrX/issues/206
...
void LZipEntry::wrapForLua(lua_State *L) {
	...
	module(L)[class_<ZipFile::ZipEntry>("ZipEntry")
...
void LZipFile::wrapForLua(lua_State *L) // Updated v5.6.34. Thanks to @dnaldoog
{
	...
	module(L)[class_<ZipFile>("ZipFile")
				  .def(constructor<const File &>())
				  ...
				  .def("createStreamForEntry", (InputStream * (ZipFile::*)(int)) & ZipFile::createStreamForEntry,
					   adopt(result))
				  .def("createStreamForEntry",
					   (InputStream * (ZipFile::*)(const ZipFile::ZipEntry &)) & ZipFile::createStreamForEntry,
					   adopt(result))
				  .def("uncompressTo", &ZipFile::uncompressTo)
				  .def("uncompressEntry", (Result (ZipFile::*)(int, const File &, bool))&ZipFile::uncompressEntry)];
}

void LInputStream::wrapForLua(
	lua_State *L) // Updated v5.6.34. Thanks to @dnaldoog. https://github.com/damiensellier/CtrlrX/pull/116
{
	using namespace luabind;

	module(L)[class_<InputStream>("InputStream")
				  .def("readEntireStreamAsString", &InputStream::readEntireStreamAsString)
				  .def("readIntoMemoryBlock",
					   (size_t (InputStream::*)(MemoryBlock &, ssize_t))&InputStream::readIntoMemoryBlock)
