	...
	if (stringIsHexadecimal(sanitizedHex)) {
		...
		if (rawData.getSize() > 0) {
			...
			if (m.getRawDataSize() > 0) {
				...
				if (m.isSysEx())
					messageType = (CtrlrMidiMessageType)1;
				else if (m.isController())
					messageType = (CtrlrMidiMessageType)2;
...
CtrlrMidiMessage::CtrlrMidiMessage(
	const luabind::object &tableData) // Added v5.6.35. Handle table to MIDI message from LUA
	: messageType(None),
	  midiTree(Ids::midi),
	  multiMasterValue(1),
	  multiMasterNumber(1),
...
CtrlrMidiMessage::CtrlrMidiMessage(const CtrlrLuaObjectWrapper &luaArray)
	...
	if (possibleMidiData.getSize() > 0) {
		...
		if (!initializationResult.wasOk()) {
			_WRN("CtrlrMidiMessage::ctor LUA can't create midi message from input data: " +
				 memoryBlockToString(possibleMidiData));
		} else {
			// Sysex is not added automagicly (since we need a valid formula), do it now
			if (messageType == SysEx) {
...
CtrlrMidiMessage::~CtrlrMidiMessage() { midiTree.removeListener(this); }
...
Result CtrlrMidiMessage::fillMessagePropertiesFromData() {
	if (messageArray.size() == 0) {
		...
		setProperty(Ids::midiMessageLatchAndStream,
					false); // Added v5.6.35. For Multi MIDI Message. Thanks to @dnaldoog
		setProperty(Ids::midiMessageMultiList, "");
		setProperty(Ids::midiMessageSysExFormula, "");

		return (Result::fail("MessageArray is empty, initializing to empty NONE message"));
	} else {
		return (fillMessagePropertiesFromData(
			MemoryBlock(messageArray[0].m.getRawData(), messageArray[0].m.getRawDataSize())));
	}
}

void CtrlrMidiMessage::restoreState(const ValueTree &savedState) {
	for (int i = 0; i < savedState.getNumProperties(); i++) {
		midiTree.setProperty(savedState.getPropertyName(i), savedState.getProperty(savedState.getPropertyName(i), 0),
							 0);
	}
	...
	if (getProperty(Ids::midiMessageSysExFormula).toString().length() > 0 &&
		(int)getProperty(Ids::midiMessageType) != SysEx)

	{
		setProperty(Ids::midiMessageSysExFormula, "");
	} else if (getProperty(Ids::midiMessageSysExFormula).toString().length() > 0 &&
			   (int)getProperty(Ids::midiMessageType) == SysEx) {
...
void CtrlrMidiMessage::setValueToSingle(const int index, const int value) {
	if (messageArray.size() > index && messageArray.getReference(index).overrideValue == -2) {
		...
		if (getSysexProcessor()) {
			getSysexProcessor()->sysExProcess(messageArray.getReference(index).getTokenArray(),
											  messageArray.getReference(index).m, value, getChannel());
		}
	} else {
...
int CtrlrMidiMessage::getValue() {
	if (messageType != Multi && messageType != SysEx) {
		...
		if (messageArray.size() == 1) {
			if (getSysexProcessor()) {
				return (getSysexProcessor()->getValueFromSysExData(messageArray.getReference(0).getTokenArray(),
																   messageArray.getReference(0)));
			} else {
				return (CtrlrSysexProcessor::getValue(messageArray.getReference(0).getTokenArray(),
													  messageArray.getReference(0)));
			}
		}
	}
...
void CtrlrMidiMessage::setMultiMessageFromString(const String &savedState) {
	...
	if (s.containsIgnoreCase("ByteValue") || s.containsIgnoreCase("MSB7bitValue") ||
		s.containsIgnoreCase("LSB7bitValue")) {
	...
	for (int i = 0; i < msgs.size(); ++i) {
		...
		// Standard messages
		if (mm.midiType.equalsIgnoreCase("CC") || mm.midiType.equalsIgnoreCase("Aftertouch") ||
			mm.midiType.equalsIgnoreCase("ChannelPressure") || mm.midiType.equalsIgnoreCase("NoteOn") ||
			mm.midiType.equalsIgnoreCase("NoteOff") || mm.midiType.equalsIgnoreCase("PitchWheel")) {
	...
	for (const auto &mm : multiMessages) {
		...
		if (mm.midiType.equalsIgnoreCase("CC")) {
			...
			if (mm.sysexData.isNotEmpty()) {
				...
				// mex.m = midiMessageExfromString(mm.sysexData, channel, componentNumber, componentValue).m; // don't
				// use this -> needs the old 6 value csv
...
void CtrlrMidiMessage::setNumberToSingle(const int index, const int number) {
	...
	if (messageArray.getReference(index).overrideValue == -2) {
		...
		// Add guard for Novation 8 bit system where MSB (Coarse) Number has 32 added to it for LSB Fine - This could go
		// out of bounds, so protected. Presuming that Coarse is sent first followed by fine 00/64 value
		auto &mex = messageArray.getReference(
			index); // Added v5.6.35. SafeGuards for Novation CC coarse/fine. Thanks to @dnaldoog
		_DBG("setNumberToSingle index=" + String(index) + " flag=" + String(mex.indirectValueFlag) +
...
CtrlrMidiMessageType CtrlrMidiMessage::getMidiMessageType() const { return (messageType); }
...
void CtrlrMidiMessage::setMidiMessageType(const CtrlrMidiMessageType newType) {
	...
	case SysEx:
		...
		if (getProperty(Ids::midiMessageSysExFormula).toString().length() > 0) {
			messageArray.add(CtrlrSysexProcessor::sysexMessageFromString(getProperty(Ids::midiMessageSysExFormula),
																		 getValue(), getChannel()));
			patternChanged();
		}
	...
	case Multi:
		...
		setMultiMessageFromString(getProperty(Ids::midiMessageMultiList)
									  .toString()); // Updated v5.6.35. For Multi MIDI Message. Thanks to @dnaldoog
		break;
	...
	case PitchWheel:
		...
		messageArray.add(MidiMessage::pitchWheel(
			ch, jmin<int>(getValue(), 16383))); // Updated v5.6.35. For Multi MIDI Message. Thanks to @dnaldoog
		break;
...
void CtrlrMidiMessage::valueTreePropertyChanged(ValueTree &treeWhosePropertyHasChanged, const Identifier &property) {
	...
	if (property == Ids::midiMessageType) {
		...
		_DBG("CtrlrMidiMessage::valueTreePropertyChanged midiMessageCtrlrValue=" + String(newVal) +
			 " messageArraySize=" + String((int)messageArray.size()));
		if (messageArray.size() > 0)
			_DBG("CtrlrMidiMessage BEFORE raw=" + String::toHexString(messageArray.getReference(0).m.getRawData(),
																	  messageArray.getReference(0).m.getRawDataSize()));
		setValue(newVal);
		if (messageArray.size() > 0)
			_DBG("CtrlrMidiMessage AFTER raw=" + String::toHexString(messageArray.getReference(0).m.getRawData(),
																	 messageArray.getReference(0).m.getRawDataSize()));
		return;
		...
		if (!getProperty(Ids::midiMessageMultiList)
				 .toString()
				 .containsIgnoreCase("ByteValue")) // Updated v5.6.35. Thanks to @dnaldoog
			setNumber((int)getProperty(Ids::midiMessageCtrlrNumber));
		setValue((int)getProperty(Ids::midiMessageCtrlrValue));
	}
...
void CtrlrMidiMessage::patternChanged() {
	...
	for (int i = 0; i < messageArray.size(); i++) {
		MemoryBlock bl(0);
		if (messageArray.getReference(i).overrideValue == -2) {
			bl = MemoryBlock(messageArray.getReference(i).m.getRawData(),
							 messageArray.getReference(i).m.getRawDataSize());
		} else {
			bl = midiMessagePattern(messageArray.getReference(i), messageArray.getReference(i).getTokenArray(),
									getGlobalVariables());
		}
...
const MemoryBlock &CtrlrMidiMessage::getMidiPattern() const { return (messagePattern); }

Array<CtrlrMidiMessageEx> &CtrlrMidiMessage::getMidiMessageArray() { return (messageArray); }

const String CtrlrMidiMessage::toString() const {
	_DBG("toString() called. Buffer Size: " + String(getData().getSize()) +
		 " Pattern Size: " + String(messagePattern.getSize()));
	return (String::toHexString(getData().getData(), getData().getSize()));
}

int CtrlrMidiMessage::getSize() const { return (getData().getSize()); }

void CtrlrMidiMessage::wrapForLua(lua_State *L) {
	using namespace luabind;

	module(
		L)[class_<CtrlrMidiMessage>("CtrlrMidiMessage")
			   ...
			   .enum_("CtrlrMidiMessageType")[value("CC", 0), value("Aftertouch", 1), value("ChannelPressure", 2),
											  value("NoteOn", 3), value("NoteOff", 4), value("SysEx", 5),
											  value("Multi", 6), value("ProgramChange", 7), value("PitchWheel", 8),
											  value("None", 9), value("MidiClock", 10), value("MidiClockContinue", 11),
											  value("MidiClockStop", 12), value("MidiClockStart", 13)]
			   ...
			   .def(
				   "toString",
				   +[](const CtrlrMidiMessage &self) -> std::string { // Updated v5.6.35. Using Lambda here is safer
					   return self.toString().toStdString();
				   })
			   .def("getInitializationResult", &CtrlrMidiMessage::getInitializationResult)
			   .def("getProperty", (const var &(CtrlrMidiMessage::*)(const Identifier &) const) &
									   CtrlrMidiMessage::getProperty) // Added v5.6.31
			   .def("setProperty",
					(void (CtrlrMidiMessage::*)(const Identifier &, const var &,
												const bool))&CtrlrMidiMessage::setProperty) // Added v5.6.33
	];
}
