...
struct DelayedPluginDeleter : private Timer, private DeletedAtShutdown {
		DelayedPluginDeleter(AudioPluginInstance *p) : plugin(p) { startTimer(1000); }
...
struct CtrlrParameterFromHost {
		CtrlrParameterFromHost(const int _index, const float _value) : index(_index), value(_value) {}
		CtrlrParameterFromHost() : index(-1), value(0.0f) {}
		int index;
		float value;
};
...
class CtrlrProcessor : public AudioProcessor, public ChangeBroadcaster {
	public:
		...
		int32 getAAXPluginIDForMainBusConfig(const AudioChannelSet &mainInputLayout,
											 const AudioChannelSet &mainOutputLayout, bool idForAudioSuite) const {
		...
		void setMidiOptions(const bool _thruHostToHost, const bool _thruHostToDevice, const bool _outputToHost,
	...
	private:
		...
		bool thruHostToHost, thruHostToDevice, thruFromHostChannelize, outputToHost, inputFromHost;
		...
		PluginLoggerVst3(const juce::File &pluginExecutableFile) {
			...
			if (fileExt == ".exe") {
				...
				logFile = File::getSpecialLocation(File::userDesktopDirectory)
							  .getChildFile("CtrlrX_debug_log.txt"); // VST3 folder is read only unless admin
			}
			if (!logFile.exists()) {
				logFile.create();
			}
