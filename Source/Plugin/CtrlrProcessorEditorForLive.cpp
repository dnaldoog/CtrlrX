...
CtrlrEditorWrapper::CtrlrEditorWrapper(CtrlrProcessorEditorForLive &_liveEditorOwner, CtrlrProcessor *ownerFilter,
	  ...
	  wrapper(*this, filterOwner, owner)					// Initialize the DocumentWindow wrapper
{
	...
	// File debugLogForLive =
	// File::getSpecialLocation(File::userDesktopDirectory).getChildFile("CtrlrX_vst3_live_debug_log.txt");
...
void CtrlrProcessorEditorForLive::wrapperResized() {
	setSize(wrapper.getWidth(),
			16); // NOTE v5.6.34. This is weird because Height will always reset to 16px when resized.
}
