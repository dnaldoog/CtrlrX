	  ...
	  lastCommandInvocationMillis(0)
{
	...
	if (isRestricted() && owner.getActivePanel())
	{
		...
		hideMidiControllerMenu =
			owner.getActivePanel()->getEditor()->getProperty(Ids::uiPanelMidiControllerMenuHideOnExport);
...
void CtrlrEditor::resized()
{
	if (menuBar->isVisible())
	{
		...
		owner.getCtrlrDocumentPanel().setBounds(0, (int)owner.getProperty(Ids::ctrlrMenuBarHeight, 24), getWidth(),
...
void CtrlrEditor::setEditorLookAndFeel(const String &lookAndFeelDesc, const var &colourSchemeProperty)
{
	...
	// DBG("*** setEditorLookAndFeel called: desc=" << lookAndFeelDesc);
	...
	if (newLookAndFeel != nullptr)
	{
		...
		// DBG("*** CtrlrEditor.cpp immediately before move: desktop=" << juce::String::toHexString(
				(juce::pointer_sized_int)&Desktop::getInstance().getDefaultLookAndFeel();
				...
				// 7. Set as desktop default so new dialogs/windows inherit this L&F
				// Guarded: only set if not already the desktop default (prevents assertion
				// when a previous holder is still being torn down)
				if (currentLookAndFeel != nullptr && &Desktop::getInstance().getDefaultLookAndFeel() != currentLookAndFeel.get())
				{
