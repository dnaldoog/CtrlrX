...
// public LookAndFeel_V4 // Removed v5.6.34 to set lnf version independently
{
	public:
		...
		enum TopMenuIDs { MenuFile, MenuEdit, MenuView, MenuPanel, MenuMidi, MenuPrograms, MenuTools, MenuHelp };
		...
		void setEditorLookAndFeel(const String &lookAndFeelDesc,
								  const juce::var &colourSchemeProperty); // Added v5.6.34

		void performLuaEditorCommand(const int commandID); // Added v5.6.34. Required to add shortcuts
														   // to LUA Method Editor menu items

		JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CtrlrEditor)

	private:
		// Use a ScopedPointer to manage the menuBar object
		// juce::std::unique_ptr<juce::MenuBarComponent> menuBar;
		std::unique_ptr<juce::MenuBarComponent> menuBar;
		// MenuBarComponent *menuBar; // Updated v5.6.34. ScopedPointed will handle its destruction
		// properly

		// Use a ScopedPointer to manage the current LookAndFeel object
		// std::unique_ptr<LookAndFeel> currentLookAndFeel; // Updated v5.6.34. ScopedPointed will handle
		// its destruction properly
		std::unique_ptr<juce::LookAndFeel> currentLookAndFeel;
		/*
		There is an invisible architectural trap that explains why it was still leaking!
		JUCE's legacy ScopedPointer deletes its managed object automatically when the parent container
		(CtrlrEditor) dies, which is good. However, look at the order of operations when CtrlrEditor
		shuts down: CtrlrEditor begins its implicit destructor phase. The std::unique_ptr<LookAndFeel>
		runs its destructor and deletes the layout memory. The Trap: Because the component hierarchy
		never explicitly called setLookAndFeel(nullptr) before that memory was deleted, the child
		components are left holding dangling raw pointers to a destroyed look-and-feel object during the
		final cleanup sweep. This completely breaks JUCE's internal weak-reference release tracking,
		freezing the cached vector graphics (DrawablePath, FillType, etc.) inside memory at the exact
		moment of exit. By changing it to std::unique_ptr and explicitly calling
		setLookAndFeel(nullptr); at the very top of your ~CtrlrEditor() destructor, you orchestrate a
