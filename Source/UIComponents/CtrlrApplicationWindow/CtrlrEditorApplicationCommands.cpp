...
void CtrlrEditor::getCommandInfo(CommandID commandID, ApplicationCommandInfo &result) {
	...
	case showMidiMonitor:
		result.setInfo("MIDI Monitor", "A small MIDI monitor that will display received and sent data", globalCategory,
					   0);
		result.addDefaultKeypress('m', ModifierKeys::commandModifier);
		break;

	case showLogViewer:
		result.setInfo("Log viewer", "You can view diagnostic messages here, useful when debugging problems",
					   globalCategory, 0);
		result.addDefaultKeypress('l', ModifierKeys::commandModifier);
		break;

	case showMidiCalculator:
		result.setInfo("MIDI Calculator", "A useful tool to translate Heximal, Binary, Decimal values", globalCategory,
					   0);
	...
	case doSearchForProperty:
		result.setInfo("Search for property", "Search for any property in the current panel or selected component",
					   panelCategory, 0);
		if (!isRestricted()) // Added v5.6.32. Disable shortcut on restricted instance
		{
	...
	case doSendSnapshot:
		result.setInfo("Send Snapshot", "Send all values from the panel as their defines MIDI messages", panelCategory,
					   0);
		result.setActive(isPanelActive());
		break;
	...
	case doRefreshPropertyLists:
		result.setInfo("Refresh property lists", "Refreshes all dynamic lists that occur in the property pane",
					   panelCategory, 0);
		result.setActive(isPanelActive());
		break;

	case doExportFileText:
		result.setInfo("Export XML", "Export panel to a XML file with no compression, might be very large",
					   panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;
	...
	case doExportFileBin:
		result.setInfo("Export binary", "Export panel to a binary file, unlike XML is unreadable but loads faster",
					   panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;
	...
	case doExportFileZBinRes:
		result.setInfo("Export compressed binary + resources",
					   "Export panel to a zlib compressed binary file with resources included", panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;

	case doExportFileInstance:
		result.setInfo("Export instance", "Exports the panel as a standalone instance (executable/loadable file)",
					   panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;
	...
	case doExportFileInstanceRestricted:
		result.setInfo(
			"Export restricted instance",
			"Exports the panel as a standalone restricted instance (executable/loadable file). It won't be editable.",
			panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;

	case doExportGenerateUID:
		result.setInfo("Re-generate UID [" + getPanelProperty(Ids::panelUID).toString() + "]",
					   "Each panel has a Unique ID that can be re-generated", panelCategory, 0);
		result.setActive(isPanelActive(true));
		break;
	...
	case optMidiInputFromHost:
		...
		result.setTicked(isPanelActive()
							 ? getActivePanel()->getMidiOptionBool((const CtrlrPanelMidiOption)optMidiInputFromHost)
							 : false);
		break;
	...
	case optMidiInputFromHostCompare:
		result.setInfo(
			"Input from host to comparator",
			"This option tells Ctrlr to route all incomming MIDI events from the MIDI host to the Comparator engine, "
			"messages that will match modulators in the panel will be treated as comming from a MIDI device",
			panelCategory, 0);
		result.setActive(!JUCEApplication::isStandaloneApp());
		result.setTicked(isPanelActive() ? getActivePanel()->getMidiOptionBool(
											   (const CtrlrPanelMidiOption)optMidiInputFromHostCompare)
										 : false);
		break;

	case optMidiOutuptToHost:
		result.setInfo("Output to plugin host",
					   "All MIDI output goes to the MIDI host (if supported by the plugin format and the host)",
					   panelCategory, 0);
		result.setActive(!JUCEApplication::isStandaloneApp());
		result.setTicked(isPanelActive()
							 ? getActivePanel()->getMidiOptionBool((const CtrlrPanelMidiOption)optMidiOutuptToHost)
							 : false);
		break;
	...
	case optMidiSnapshotOnProgramChange:
		result.setInfo("Snapshot on program change",
					   "After each program change event on the panel a Snapshot will be sent", panelCategory, 0);
		result.setActive(isPanelActive());
		result.setTicked(getPanelProperty(Ids::panelMidiSnapshotAfterProgramChange));
		break;

	case optMidiThruD2D:
		result.setInfo("Input device -> Output device",
					   "Send all MIDI messages comming from the MIDI input device via the MIDI output device",
					   panelCategory, 0);
		result.setActive(isPanelActive());
		if (isPanelActive())
	...
	case optMidiThruD2H:
		result.setInfo("Input device -> Plugin host",
					   "Send all MIDI messages from the MIDI input device to the plugin host (if supported by the "
					   "plugin format and the host)",
					   panelCategory, 0);
	...
	case optMidiThruH2D:
		result.setInfo("Plugin host -> Output device",
					   "Send all MIDI messages comming from the plugin host the MIDI output device", panelCategory, 0);
		result.setActive(isPanelActive() && (JUCEApplication::isStandaloneApp() == false));
		if (isPanelActive())
	...
	case optMidiThruH2H:
		result.setInfo("Plugin host -> Plugin host",
					   "Send all MIDI messages comming from the plugin host back to it (if supported by the plugin "
					   "format and the host)",
					   panelCategory, 0);
		result.setActive(isPanelActive() && (JUCEApplication::isStandaloneApp() == false));
		if (isPanelActive())
	...
	case optMidiThruD2DChannelize:
		result.setInfo("Change channel: Input device -> Output device",
					   "Change the channel of MIDI messages comming in on the MIDI input device to the MIDI channel "
					   "set for the output device",
					   panelCategory, 0);
		result.setActive(isPanelActive() && (JUCEApplication::isStandaloneApp() == false));
		if (isPanelActive())
	...
	case optMidiThruD2HChannelize:
		result.setInfo("Change channel: Input device -> Plugin host",
					   "Change the channel of MIDI messages comming in on the MIDI input device to the MIDI channel "
					   "set for the output to plugin host",
					   panelCategory, 0);
		result.setActive(isPanelActive() && (JUCEApplication::isStandaloneApp() == false));
		if (isPanelActive())
	...
	case optMidiThruH2DChannelize:
		result.setInfo("Change channel: Plugin host -> Output device",
					   "Change the channel of MIDI messages comming in from the plugin host to the MIDI channel set "
	...
	case optMidiThruH2HChannelize:
		result.setInfo("Change channel: Plugin host -> Plugin host",
					   "Change the channel of MIDI messages comming in from the plugin host to the MIDI channel set "
					   "for the output to plugin host",
					   panelCategory, 0);
		result.setActive(isPanelActive() && (JUCEApplication::isStandaloneApp() == false));
		if (isPanelActive())
	...
	case doDumpVstTables:
		result.setInfo("Dump Memory Info", "Dump information stored in memory in some readable format", globalCategory,
					   0);
		result.setActive(true);
		break;
	...
	case LuaMethodEditorCommandIDs::fileSaveAndCompile:
		...
		result.addDefaultKeypress(CharacterFunctions::toUpperCase('s'),
	...
	case LuaMethodEditorCommandIDs::editFindAndReplace:
		...
		result.addDefaultKeypress('r', ModifierKeys::commandModifier); // cmd+h was not a good idea since cmd+h is to
																	   // hide the app for macOS, cmd+r is better
		result.setActive(isPanelActive());
		break;
	...
	// added to fix crash on disable luaClassBrowser menu item. It seems that if a commandID is not handled here, the
	// app crashes when trying to show it in the menu. This way, we at least show it as a disabled item instead of
	// crashing.
...
CtrlrEditor::getNextCommandTarget() // Added v5.6.34. Required for the LUA method manager combo shortcuts
{
	...
	if (CtrlrPanel *activePanel = getActivePanel()) {
		...
		juce::Component *luaEditorContent =
			activePanel->getPanelWindowManager().getContent(CtrlrPanelWindowManager::LuaMethodEditor);
