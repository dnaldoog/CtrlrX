...
// ====
CtrlrFixedSlider::CtrlrFixedSlider(CtrlrModulator& owner) : CtrlrComponent(owner), ctrlrSlider(0) {
...
void CtrlrFixedSlider::sliderContentChanged() {
    ...
    if (values.isNotEmpty()) {
        ...
        _DBG("LOADING SLIDER: " + owner.getProperty(Ids::name).toString() + " Min: " + String(min) +
             " Max: " + String(max));
        if (max <= min) {
...
void CtrlrFixedSlider::valueTreePropertyChanged(ValueTree& treeWhosePropertyHasChanged,
...
LookAndFeel* CtrlrFixedSlider::getLookAndFeelFromComponentProperty(
