...
// ====
CtrlrImageSlider::CtrlrImageSlider(CtrlrModulator& owner) : CtrlrComponent(owner), ctrlrSlider(0) {
...
double CtrlrImageSlider::getComponentMaxValue() { return ((int)ctrlrSlider->getMaximum()); }

double CtrlrImageSlider::getComponentValue() { return ((int)ctrlrSlider->getValue()); }

int CtrlrImageSlider::getComponentMidiValue() { return ((int)ctrlrSlider->getValue()); }
...
void CtrlrImageSlider::setComponentValue(const double newValue, const bool sendChangeMessage) {
    ...
    if (sendChangeMessage) {
        owner.getProcessor().setValueGeneric(
            CtrlrModulatorValue(newValue, CtrlrModulatorValue::changedByGUI));
    }
}

void CtrlrImageSlider::valueTreePropertyChanged(ValueTree& treeWhosePropertyHasChanged,
    ...
    if (property == Ids::uiImageSliderResource) {
    ...
    } else if (property == Ids::uiSliderInterval || property == Ids::uiSliderMax ||
               property == Ids::uiSliderMin) {
        ...
        ctrlrSlider->setColour(Slider::textBoxTextColourId,
                               VAR2COLOUR(getProperty(Ids::uiSliderValueTextColour)));
    } else if (property == Ids::uiSliderValueBgColour) {
        ctrlrSlider->setColour(Slider::textBoxBackgroundColourId,
                               VAR2COLOUR(getProperty(Ids::uiSliderValueBgColour)));
        ...
        ctrlrSlider->setTextBoxStyle(
            (Slider::TextEntryBoxPosition)(int)getProperty(Ids::uiSliderValuePosition), false,
            getProperty(Ids::uiSliderValueWidth, 64), getProperty(Ids::uiSliderValueHeight, 12));
    }

    else if (property == Ids::uiSliderIncDecButtonColour ||
             property == Ids::uiSliderIncDecTextColour || property == Ids::uiSliderValueFont ||
        ...
        ctrlrSlider->setSliderStyle(
            (Slider::SliderStyle)CtrlrComponentTypeManager::sliderStringToStyle(
                getProperty(Ids::uiSliderStyle)));
        ...
        ctrlrSlider->setVelocityModeParameters(
            (double)getProperty(Ids::uiSliderVelocitySensitivity),
        ...
        ctrlrSlider->setPopupDisplayEnabled((bool)getProperty(property),
...
const String CtrlrImageSlider::getComponentText() { return (String(getComponentValue())); }

void CtrlrImageSlider::setResource() {
    filmStripImage = owner.getOwnerPanel().getResourceManager().getResourceAsImage(
        getProperty(Ids::uiImageSliderResource));
    lf->setImage(filmStripImage, (int)getProperty(Ids::resourceImageWidth),
                 (int)getProperty(Ids::resourceImageHeight));
    lookAndFeelChanged();
    repaint();
...
void CtrlrImageSlider::reloadResources(Array<CtrlrPanelResource*> resourcesThatChanged) {
    for (int i = 0; i < resourcesThatChanged.size(); i++) {
        if (resourcesThatChanged[i]->getName() ==
            getProperty(Ids::uiImageSliderResource).toString()) {
            setResource();
        }
    }
}
...
CtrlrImageSlider::CtrlrImageSlider(CtrlrModulator& owner) : CtrlrComponent(owner), ctrlrSlider(0) {
    ...
    setProperty(Ids::uiSliderValueTextJustification, "centred");
    setProperty(Ids::uiSliderValueFont, FONT2STR(Font(12)));
    setProperty(Ids::uiSliderValueTextColour,
                (String)findColour(Slider::textBoxTextColourId).toString());
    setProperty(Ids::uiSliderValueHighlightColour,
                (String)findColour(Slider::textBoxHighlightColourId).toString());
    setProperty(
        Ids::uiSliderValueBgColour,
...
CtrlrImageSlider::~CtrlrImageSlider() { deleteAndZero(ctrlrSlider); }
...
void CtrlrImageSlider::valueTreePropertyChanged(ValueTree& treeWhosePropertyHasChanged,
    ...
    if (property == Ids::uiSliderSetNotificationOnlyOnRelease) {
        ctrlrSlider->setChangeNotificationOnlyOnRelease(
            (bool)getProperty(Ids::uiSliderSetNotificationOnlyOnRelease));
    } else if (property == Ids::uiSliderLookAndFeel) {
        String LookAndFeelType = getProperty(property);
        setLookAndFeel(getLookAndFeelFromComponentProperty(
            LookAndFeelType)); // Updates the current component LookAndFeel
...
LookAndFeel* CtrlrImageSlider::getLookAndFeelFromComponentProperty(
...
void CtrlrImageSlider::resetLookAndFeelOverrides() {
        ...
        false) // To prevent the prop lines stacking up from top and keeping their original position
    {
        ...
        setProperty(
            ...
            "0x00ffffff"); // (String)findColour (Slider::textBoxBackgroundColourId).toString());
        setProperty(
            Ids::uiSliderValueOutlineColour,
            "0x00ffffff"); //(String)findColour (Slider::textBoxOutlineColourId).toString());
