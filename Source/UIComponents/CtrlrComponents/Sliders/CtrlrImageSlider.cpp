...
CtrlrImageSlider::CtrlrImageSlider(CtrlrModulator &owner) : CtrlrComponent(owner), ctrlrSlider(0) {
	...
	ctrlrSlider->setLookAndFeel(lf.get()); // V5.6.28 and before was LookAndFeel_V2, updated to V4
...
void CtrlrImageSlider::valueTreePropertyChanged(ValueTree &treeWhosePropertyHasChanged, const Identifier &property) {
	if (property == Ids::uiSliderSetNotificationOnlyOnRelease) {
		...
		// 1. Unlink the current style from JUCE first to prevent collisions
		setLookAndFeel(nullptr);

		if (LookAndFeelType == "Default") {
			// 2. Clear out our smart pointer container (safely deletes old V4 assets)
			customLF.reset();

			setProperty(Ids::uiSliderLookAndFeelIsCustom, false); // Resets the Customized Flag to False
		} else {
			// 3. Allocate the new LookAndFeel directly into our tracking manager
			customLF = getLookAndFeelFromComponentProperty(LookAndFeelType);

			// 4. Safely expose the underlying raw memory address to JUCE using .get()
			if (customLF != nullptr) {
...
CtrlrImageSlider::getLookAndFeelFromComponentProperty(const String &lookAndFeelComponentProperty) // Updated v5.6.34
{
