...
class CtrlrSliderLookAndFeel_V1 : public CtrlrSliderLookAndFeelBase {
	public:
		...
		void drawLinearSliderBackground(juce::Graphics &g, int x, int y, int w, int h, float pos, float min, float max,
										const juce::Slider::SliderStyle s, juce::Slider &sl) override {
			getNativeV1().drawLinearSliderBackground(g, x, y, w, h, pos, min, max, s, sl);
		}

		void drawLinearSliderThumb(juce::Graphics &g, int x, int y, int w, int h, float pos, float min, float max,
								   const juce::Slider::SliderStyle s, juce::Slider &sl) override {
