#include "CtrlrInlineUtilitiesGUI.h" // for helper function MyPopupHelper
#include "stdafx.h"
/*
  ==============================================================================
...
//==============================================================================
CtrlrXYSurface::CtrlrXYSurface(CtrlrModulator &owner) : CtrlrComponent(owner), surfaceModulator(0) {
...
void CtrlrXYSurface::paint(Graphics &g) {
	...
	gradientFromProperty(g, usableRectangle.getSmallestIntegerContainer(), getObjectTree(),
						 Ids::uiXYSurfaceBgGradientType, Ids::uiXYSurfaceBackgroundColour1,
						 Ids::uiXYSurfaceBackgroundColour2);
	g.fillRoundedRectangle((float)usableRectangle.getTopLeft().getX(), (float)usableRectangle.getTopLeft().getY(),
						   (float)usableRectangle.getWidth(), (float)usableRectangle.getHeight(),
						   getProperty(Ids::uiXYSurfaceCornerSize));

	gradientFromProperty(g, usableRectangle.getSmallestIntegerContainer(), getObjectTree(),
						 Ids::uiXYSurfaceOutlineGradientType, Ids::uiXYSurfaceOutlineColour1,
						 Ids::uiXYSurfaceOutlineColour2);
	g.drawRoundedRectangle((float)usableRectangle.getX(), (float)usableRectangle.getY(),
						   (float)usableRectangle.getWidth(), (float)usableRectangle.getHeight(),
						   getProperty(Ids::uiXYSurfaceCornerSize), getProperty(Ids::uiXYSurfaceOutlineThickness));
	...
	if (snapSize > 0) {
		...
		g.fillRoundedRectangle((float)usableRectangle.getX(), (float)usableRectangle.getY(),
							   (float)usableRectangle.getWidth(), (float)usableRectangle.getHeight(),
							   getProperty(Ids::uiXYSurfaceCornerSize));
	}
	...
	if (backgroundImage.isValid()) {
		if ((int)getProperty(Ids::uiXYSurfaceBgImageLayout) == 8192) {
			...
			g.fillRect((float)usableRectangle.getX(), (float)usableRectangle.getY(), (float)usableRectangle.getWidth(),
					   (float)usableRectangle.getHeight());
		} else {
			g.setColour(Colours::black.withAlpha((float)getProperty(Ids::uiXYSurfaceBgImageAlpha) / 255.0f));
			g.drawImageWithin(backgroundImage, usableRectangle.getX(), usableRectangle.getX(),
							  usableRectangle.getWidth(), usableRectangle.getHeight(),
	...
	if (getProperty(Ids::uiXYSurfaceXTrackEnabled)) {
		...
		g.drawLine((float)c.getX(), (float)usableRectangle.getY(), (float)c.getX(), (float)usableRectangle.getBottom(),
				   getProperty(Ids::uiXYSurfaceYTrackThickness));

		g.setColour(VAR2COLOUR(getProperty(Ids::uiXYSurfaceXTrackColour)));
		g.drawLine((float)usableRectangle.getX(), (float)c.getY(), (float)usableRectangle.getWidth(), (float)c.getY(),
				   getProperty(Ids::uiXYSurfaceXTrackThickness));
	}
	...
	if ((bool)getProperty(Ids::uiXYSurfaceInfoLabelVisible)) {
		...
		g.drawText("X=" + String(getValueForPosition(c.getX(), true)) +
					   " Y=" + String(getValueForPosition(c.getY(), false)),
				   usableRectangle.getX() + 8, usableRectangle.getY() + 8, usableRectangle.getWidth() - 16,
				   usableRectangle.getHeight() - 16,
				   justificationFromProperty(getProperty(Ids::uiXYSurfaceInfoLabelLocation)), true);
	}
...
void CtrlrXYSurface::resized() {
	...
	surfaceModulator->setSize(getProperty(Ids::uiXYSurfaceModulatorWidth),
							  getProperty(Ids::uiXYSurfaceModulatorHeight));
...
double CtrlrXYSurface::getComponentValue() { return (1); }

int CtrlrXYSurface::getComponentMidiValue() { return (1); }

double CtrlrXYSurface::getComponentMaxValue() { return (1); }
...
void CtrlrXYSurface::modulatorMoved() {
	...
	if (destinationX) {
		destinationX->getProcessor().setValueGeneric(
			CtrlrModulatorValue(getValueForPosition(c.getX(), true), CtrlrModulatorValue::changedByGUI));
	}

	if (destinationY) {
		destinationY->getProcessor().setValueGeneric(
			CtrlrModulatorValue(getValueForPosition(c.getY(), false), CtrlrModulatorValue::changedByGUI));
	}
...
void CtrlrXYSurface::setResource() {
	backgroundImage =
		owner.getOwnerPanel().getResourceManager().getResourceAsImage(getProperty(Ids::uiXYSurfaceBgImageResource));
	repaint();
	resized();
}
