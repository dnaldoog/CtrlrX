	...
	public:
		...
		void paintListBoxItem(int rowNumber, juce::Graphics &g, int width, int height, bool rowIsSelected) override {
			...
			case TypeClass:
				...
				break;
			case TypeMethod:
				iconColor = juce::Colours::darkcyan;
				iconLetter = "M";
				break;
			case TypeStatic:
				iconColor = juce::Colours::darkblue;
				iconLetter = "S";
				break;
			case TypeGlobal:
				iconColor = juce::Colours::darkgreen;
				iconLetter = "V";
				break;
			case TypeUtility:
				iconColor = juce::Colours::purple;
				iconLetter = "f";
				break;
			case TypeProperty:
				iconColor = juce::Colours::darkgoldenrod;
				iconLetter = "P";
				break;
			default:
				iconColor = mainText.withAlpha(0.5f);
				iconLetter = "?";
				break;
			}
			...
			if (params.isNotEmpty()) {
				...
				g.drawText(params, textX + methodWidth + 4, 0, width - textX - methodWidth - 10, height,
...
CtrlrLuaMethodCodeEditor::CtrlrLuaMethodCodeEditor(CtrlrLuaMethodEditor &_owner, CtrlrLuaMethod *_method,
	...
	addAndMakeVisible(editorComponent =
						  new GenericCodeEditorComponent(*this, document, codeTokeniser = new CtrlrLuaCodeTokeniser()));
	...
	suggestionPopup =
		std::make_unique<LuaSuggestionPopup>([this](const SuggestionItem &item) { handleSuggestionChosen(item); });

	addChildComponent(suggestionPopup.get());

	// 3. Search Tabs Logic
	hiddenSearchTabsToggle = std::make_unique<ToggleButton>("Hidden Search Tabs");
	hiddenSearchTabsToggle->setVisible(false);
	addAndMakeVisible(hiddenSearchTabsToggle.get());
	...
	editorComponent->setColour(CodeEditorComponent::backgroundColourId,
							   Colours::white); // findColour(CodeEditorComponent::backgroundColourId));
	editorComponent->setColour(CodeEditorComponent::defaultTextColourId,
							   Colours::black); // findColour(CodeEditorComponent::defaultTextColourId));
	editorComponent->setColour(CodeEditorComponent::highlightColourId,
							   findColour(CodeEditorComponent::highlightColourId));
	editorComponent->setColour(CodeEditorComponent::lineNumberTextId,
							   findColour(CodeEditorComponent::lineNumberTextId));
	...
	if (owner.getComponentTree().hasProperty(Ids::luaMethodEditorFont)) {
		setFontAndColour(
			owner.getOwner().getCtrlrManagerOwner().getFontManager().getFontFromString(
				owner.getComponentTree().getProperty(Ids::luaMethodEditorFont)),
			VAR2COLOUR(owner.getComponentTree().getProperty(
				Ids::luaMethodEditorBgColour,
				Colours::white
					.toString()))); // (String) findColour(CodeEditorComponent::backgroundColourId).toString())));

		editorComponent->setColour(
			CodeEditorComponent::lineNumberTextId,
			VAR2COLOUR(owner.getComponentTree().getProperty(Ids::luaMethodEditorLineNumbersColour,
															Colours::grey.toString()))); // Added v5.6.31

		editorComponent->setColour(
			CodeEditorComponent::lineNumberBackgroundId,
			VAR2COLOUR(owner.getComponentTree().getProperty(Ids::luaMethodEditorLineNumbersBgColour,
															Colours::cyan.toString()))); // Added v5.6.31
	} else {
		setFontAndColour(Font(Font::getDefaultMonospacedFontName(), 14.0f, Font::plain),
						 Colours::white); // findColour(CodeEditorComponent::backgroundColourId));
	}
}
...
void CtrlrLuaMethodCodeEditor::resized() { editorComponent->setBounds(0, 0, getWidth(), getHeight()); }
...
bool CtrlrLuaMethodCodeEditor::keyStateChanged(bool isKeyDown, Component *originatingComponent) {
	...
	if (isKeyDown) {
		...
		owner.setPositionLabelText("Line:  " + String(pos.getLineNumber() + 1) +
								   " Column: " + String(pos.getIndexInLine()));
...
void CtrlrLuaMethodCodeEditor::updateCallTipHighlight() {
	...
	while (methodStart > 0 &&
		   (juce::CharacterFunctions::isLetterOrDigit(allText[methodStart - 1]) || allText[methodStart - 1] == '_'))
		methodStart--;
...
void CtrlrLuaMethodCodeEditor::codeDocumentTextInserted(const juce::String &newText, int insertIndex) {
	...
	if (separator == ':' || separator == '.') {
		...
		expressionToResolve = document
								  .getTextBetween(juce::CodeDocument::Position(document, searchPos),
												  juce::CodeDocument::Position(document, searchOrigin))
								  .trim();
		...
		if (expressionToResolve.isNotEmpty()) {
			...
			if (className.isEmpty()) {
				if (expressionToResolve.equalsIgnoreCase("panel") || expressionToResolve == "pan")
					className = "CtrlrPanel";
				else if (expressionToResolve.startsWithIgnoreCase("mod"))
					className = "CtrlrModulator";
				else if (expressionToResolve == "utils")
					className = "utils";
			}
		}
		...
		if (className.isNotEmpty()) {
			...
			matches = manager.getMethodSuggestionsForClass(className, currentWord, includeInstance, includeStatic,
														   includeProperties);
...
void CtrlrLuaMethodCodeEditor::saveDocument() {
	...
	if (method) {
		...
		if (hasChanged && !method->isSourceInFile()) { // This method is stored within the panel => set the panel dirty
...
const bool CtrlrLuaMethodCodeEditor::isMouseOverUrl(CodeDocument::Position &position, String *url) {
	...
	const String word =
		document.getTextBetween(CodeDocument::Position(document, start), CodeDocument::Position(document, end)).trim();
...
void CtrlrLuaMethodCodeEditor::handleAsyncUpdate() { owner.tabChanged(this, false, false); }
...
void CtrlrLuaMethodCodeEditor::findNextMatch(const String &search, bool bMatchCase) {
	...
	if (position >= 0) {
		...
		if (editorComponent) {
			editorComponent->selectRegion(CodeDocument::Position(document, lastFoundPosition),
										  CodeDocument::Position(doc, lastFoundPosition + search.length()));
		}
	} else {
		lastFoundPosition = -1;
	}
...
void CtrlrLuaMethodCodeEditor::findInOpened(const String &search) {
	...
	for (int i = 0; i < owner.getTabs()->getNumTabs(); i++) {
		CtrlrLuaMethodCodeEditor *codeEditor =
			dynamic_cast<CtrlrLuaMethodCodeEditor *>(owner.getTabs()->getTabContentComponent(i));
...
bool isValidSearchString(const String &search) {
	...
	StringArray commonInvalidSearches = {
		"a",  "an", "and", "the", "of", "to", "in", "it", "is", "be", "as", "at", "by", "for", "with", "on", "1",  "2",
		"3",  "4",	"5",   "6",	  "7",	"8",  "9",	"0",  "aa", "bb", "cc", "dd", "ee", "ff",  "gg",   "hh", "ii", "jj",
		"kk", "ll", "mm",  "nn",  "oo", "pp", "11", "22", "33", "44", "55", "66", "77", "88",  "99",   "00"};
...
void CtrlrLuaMethodCodeEditor::findInAll(const String &search) {
	...
	if (!isValidSearchString(search)) {
		owner.getMethodEditArea()->insertOutput("\n\nInvalid search term: \"" + search +
													"\". Please enter a meaningful search term (at least 3 characters, "
													"not just whitespace or common single characters).\n",
												Colours::red);
		return;
	}
		...
		if (m) {
			...
			} else // Method is not yet opened
			{
				...
				if (!shouldOpenTabs || results.size() == 0) {
					...
					DBG("Closing tab for method: " + names[i] + " (shouldOpenTabs=" +
						String(shouldOpenTabs ? "true" : "false") + ", results=" + String(results.size()) + ")");
					// Close the tab if:
					...
					DBG("Keeping tab open for method: " + names[i] + " (shouldOpenTabs=" +
						String(shouldOpenTabs ? "true" : "false") + ", results=" + String(results.size()) + ")");
				}
...
//     {
//         owner.getMethodEditArea()->insertOutput("\n\nInvalid search term: \"" + search +
//             "\". Please enter a meaningful search term (at least 3 characters, not just whitespace or common single
//             characters).\n", Colours::red);
//         return;
//     }
//     owner.getMethodEditArea()->insertOutput("\n\nSearching for: \""+search+"\" in all methods (double click line to
//     jump)\n", Colours::darkblue); StringArray names;
//
//     for (int i=0; i<owner.getMethodManager().getNumMethods(); i++)
//     {
//         CtrlrLuaMethod *m = owner.getMethodManager().getMethodByIndex (i);
...
void CtrlrLuaMethodCodeEditor::gotoLine(int position, const bool selectLine) {
	...
	if (selectLine) {
		editorComponent->selectRegion(CodeDocument::Position(document, position - 1, 0),
									  CodeDocument::Position(document, position - 1, 1024));
	}
	// editorComponent->selectRegion(CodeDocument::Position(document, position - 1, 0), CodeDocument::Position(document,
	// position - 1, 1));
	editorComponent->grabKeyboardFocus();
	editorComponent->hideGoTOPanel();
}

CtrlrLuaMethodEditor &CtrlrLuaMethodCodeEditor::getOwner() { return (owner); }

//==============================================================================
class GenericCodeEditorComponent::GoToPanel : public Component, private TextEditor::Listener, private Button::Listener {
	public:
...
class GenericCodeEditorComponent::FindPanel : public Component,
	...
	public:
		...
		void applyCurrentSetting() {
			if (GenericCodeEditorComponent *ed = getOwner()) {
				caseButton.setToggleState(ed->getCtrlrLuaMethodCodeEditor().getCtrlrLuaMethodEditor().caseCansitive,
										  dontSendNotification);
				lookInComboBox.setText(ed->getCtrlrLuaMethodCodeEditor().getCtrlrLuaMethodEditor().lookInString,
									   dontSendNotification);
				searchInComboBox.setText(ed->getCtrlrLuaMethodCodeEditor().getCtrlrLuaMethodEditor().searchInString,
										 dontSendNotification);
		...
		void comboBoxChanged(ComboBox *combo) override {
			if (combo == &searchInComboBox) {
				...
				if (GenericCodeEditorComponent *ed = getOwner())
					ed->getCtrlrLuaMethodCodeEditor().getCtrlrLuaMethodEditor().lookInString = lookInComboBox.getText();
			}
		}
		...
		// For replace
		std::unique_ptr<TextEditor> replaceEditor;
		std::unique_ptr<Label> replaceLabel;
		std::unique_ptr<TextButton> replaceButton, replaceAllButton;
};

//==============================================================================
GenericCodeEditorComponent::GenericCodeEditorComponent(CtrlrLuaMethodCodeEditor &_owner, CodeDocument &codeDocument,
													   CodeTokeniser *tokeniser)
	: CodeEditorComponent(codeDocument, tokeniser), owner(_owner), bSensitive(false), lookUpString("") {
...
bool GenericCodeEditorComponent::isFindActive() { return (findPanel == nullptr); }
...
void GenericCodeEditorComponent::hideGoTOPanel() { goToPanel = nullptr; }
...
void GenericCodeEditorComponent::findNext(bool forwards, bool skipCurrentSelection) {
	...
	const CodeDocument::Position startPos(getDocument(),
										  skipCurrentSelection ? highlight.getEnd() : highlight.getStart());
	...
	for (int linesToSearch = totalLines; --linesToSearch >= 0;) {
		...
		if (forwards) {
			index = caseSensitive ? line.indexOf(linePos, searchText) : line.indexOfIgnoreCase(linePos, searchText);
		} else {
			if (linePos >= 0)
				line = line.substring(0, linePos);

			index = caseSensitive ? line.lastIndexOf(searchText) : line.lastIndexOfIgnoreCase(searchText);
		}
...
void GenericCodeEditorComponent::gotoLine(int position) { owner.gotoLine(position); }
...
void CtrlrLuaMethodCodeEditor::toggleLongLineComment() {
	...
	if (isBlockCommented) {
		...
		if (startCommentPos >= 0) {
			uncommentedText =
				uncommentedText.substring(0, startCommentPos) + uncommentedText.substring(startCommentPos + 4);
		}
		...
		if (endCommentPos >= 0) {
			uncommentedText =
				uncommentedText.substring(0, endCommentPos) + uncommentedText.substring(endCommentPos + 4);
		}
...
juce::String CtrlrLuaMethodCodeEditor::getWordBeforeCaret(int &startOfWord,
														  int offset) // Added v5.6.35. Autocomplete typing feature
{
...
void CtrlrLuaMethodCodeEditor::performReplacement(const juce::String &suggestion, bool triggerMethods,
	...
	if (wordStart > 0) {
		...
		if (separator == '.' || separator == ':') {
			...
			while (varStart > 0 &&
				   (juce::CharacterFunctions::isLetterOrDigit(allText[varStart - 1]) || allText[varStart - 1] == '_'))
				varStart--;
	...
	if (shouldAddParentheses || methodParams.isNotEmpty()) {
		...
		if (methodParams.trim().isNotEmpty() && methodParams.trim() != "()") {
			...
			juce::String cleaned =
				simplestLine.replace("void ", "").replace("size_t ", "").replace("luabind::object ", "table ", false);
			...
			for (int i = 0; i < params.size(); ++i) {
				...
				if (p.isNotEmpty()) {
					if (validParamCount > 0)
						suffix += ", ";
					if (p.containsIgnoreCase("String") || p.containsIgnoreCase("name") ||
						p.containsIgnoreCase("text")) {
						suffix += "\"\"";
						if (validParamCount == 0)
							firstParamIsString = true;
					}
					validParamCount++;
				}
			}
			...
			bool isGetter =
				cleanSuggestion.startsWithIgnoreCase("get") && !cleanSuggestion.containsIgnoreCase("ByName");
