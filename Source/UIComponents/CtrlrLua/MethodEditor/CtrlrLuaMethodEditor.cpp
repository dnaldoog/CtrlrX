...
CtrlrLuaMethodEditor::CtrlrLuaMethodEditor(CtrlrPanel &_owner)
	...
	methodTree = std::make_unique<CtrlrValueTreeEditorTree>("LUA METHOD TREE", owner);
	addAndMakeVisible(methodTree.get());
...
CtrlrLuaMethodEditor::~CtrlrLuaMethodEditor() {
	...
	// 1. Tell JUCE to clear any active tree nodes while the object is still alive
	if (methodTree != nullptr)
		methodTree->deleteRootItem();

	// 2. Erase the old deleteAndZero macro line completely.
	// Calling .reset() tells the unique_ptr to cleanly invoke ~CtrlrValueTreeEditorTree()
	methodTree.reset();
...
void CtrlrLuaMethodEditor::resized() {
	Component *comps[] = {methodTree.get(), resizer, methodEditArea};
...
void CtrlrLuaMethodEditor::tabChanged(CtrlrLuaMethodCodeEditor *codeEditor, const bool save, const bool recompile) {
	...
	if (codeEditor->getMethod() && (recompile || save)) {
		...
		// codeEditor->getMethod()->isValid() ? Colours::green : Colours::red);
		methodEditArea->insertOutput(codeEditor->getMethod()->getLastError());
	}
...
const AttributedString CtrlrLuaMethodEditor::getDisplayString(const ValueTree &item) const {
	...
	if (item.getType() == Ids::luaMethod) {
		...
		if ((int)item.getProperty(Ids::luaMethodSource) == CtrlrLuaMethod::codeInFile) {
			// str.append (File::descriptionOfSizeInBytes
			// (owner.getLuaMethodSourceFile(&item).getSize()), fSmall, text.brighter(0.2f)); //
			// Removed v5.6.31
	...
	if (item.getType() == Ids::luaMethodGroup) {
		...
		str.append(" [" + String(item.getNumChildren()) + "]", fSmall,
				   Colours::darkgrey); // Added v5.6.31
...
StringArray CtrlrLuaMethodEditor::getMenuBarNames() {
	const char *const names[] = {"File", "Edit",
								 nullptr}; // Removed 5.6.34. Help is now useless since we have the shortcuts
										   // displayed in the menu items. nullptr is mandatory to tell the
										   // process to stop the at the last member of array.
...
void CtrlrLuaMethodEditor::menuItemSelected(int menuItemID,
											int topLevelMenuIndex) // Updated v5.6.34.
{
	// This function should be empty or handle only menu items NOT registered with the
	// ApplicationCommandManager. The command manager will handle the rest.
	switch (menuItemID) {
...
void CtrlrLuaMethodEditor::searchResultClicked(const String &methodName, const int lineNumber,
	...
	//    _DBG("\t"+methodName+" ln:"+STR(lineNumber)+" s:"+STR(resultPositionStart)+"
	//    e:"+STR(resultPositionEnd));
