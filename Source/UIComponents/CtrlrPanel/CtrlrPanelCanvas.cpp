...
CtrlrPanelCanvas::~CtrlrPanelCanvas()
{
	...
	getOwner().getPanelEditorTree().removeListener(this); // this is accessing freed memory. Canvas has to be deleted before valueTree, but not sure where...
...
void CtrlrPanelCanvas::importImage(const File &imageFile, int x, int y)
{
	if (owner.getOwner().getResourceManager().addResource(imageFile))
	{
		...
		AlertWindow::showMessageBoxAsync(AlertWindow::InfoIcon, "Image import", "Can't import the file as a image resource");
#else
	...
	{
#if JUCE_VERSION >= 0x070000
		AlertWindow::showMessageBoxAsync(AlertWindow::InfoIcon, "Image import", "Image has been imported as a resource, but i can't fetch if from the resource manager");
#else
