...
void CtrlrPanelCanvas::paint(Graphics &g) {
	...
	if ((int)getOwner().getProperty(Ids::uiPanelBackgroundGradientType) == 0) {
		...
		gradientFromProperty(g, getBounds(), getOwner().getObjectTree(), Ids::uiPanelBackgroundGradientType,
							 Ids::uiPanelBackgroundColour1, Ids::uiPanelBackgroundColour2);
		g.fillAll();
	}

	if (ctrlrPanelBackgroundImage.isValid()) {
		if ((int)getOwner().getProperty(Ids::uiPanelImageLayout) == 8192) {
			g.setTiledImageFill(ctrlrPanelBackgroundImage, 0, 0,
								(float)getOwner().getProperty(Ids::uiPanelImageAlpha) / 255.0f);
			g.fillAll();
		} else {
			g.drawImageWithin(ctrlrPanelBackgroundImage, 0, 0, getWidth(), getHeight(),
	...
	if (getOwner().getProperty(Ids::uiPanelEditMode)) {
		...
		lassoComponent.setColour(decltype(lassoComponent)::ColourIds::lassoOutlineColourId,
								 findColour(PopupMenu::highlightedBackgroundColourId));
	}
	//[/UserPaint]
}
...
void CtrlrPanelCanvas::mouseDown(const MouseEvent &e) {
	...
	if (!isLayer(e.eventComponent) && editMode && !e.mods.isCommandDown() && !e.mods.isShiftDown() &&
		!e.mods.isPopupMenu()) {
	...
	if (!isLayer(e.eventComponent) && editMode && getOwner().getSelection()) {
		if (getOwner().getSelection()->getNumSelected() > 1) {
			if (e.mods.isShiftDown()) {
				dragSelectedComps(e.getDistanceFromDragStartX(), e.getDistanceFromDragStartY(),
								  (bool)getProperty(Ids::uiPanelSnapActive));
			} else {
				return;
			}
		}
...
bool CtrlrPanelCanvas::keyPressed(const KeyPress &key, Component *originatingComponent) {
	...
	if (getPanel().getEditMode()) { // Only consume copy/paste shortcuts if edit mode is activated
...
CtrlrComponent *CtrlrPanelCanvas::attach(CtrlrComponent *ctrlrComponent, Component *componentToAddTo,
										 const bool force) {
...
CtrlrComponent *CtrlrPanelCanvas::addNewComponent(const Identifier &componentType, const Point<int> insertAt,
												  Component *componentToAddTo) {
...
CtrlrComponent *CtrlrPanelCanvas::addNewComponent(const Identifier &componentType,
...
CtrlrComponent *CtrlrPanelCanvas::addNewComponent(const ValueTree &savedState, Component *componentToAddTo,
												  const bool force) {
	...
	if (savedState.getChildWithName(Ids::component).isValid()) {
		ctrlrComponentPosition =
			VAR2RECT(savedState.getChildWithName(Ids::component).getProperty(Ids::componentRectangle, "0 0 32 32"));
	} else {
		_ERR("CtrlrPanelCanvas::addNewComponent a modulator without a component was attempted, not possible when "
			 "adding from a ValueTree");
		return (nullptr);
	}
	...
	if (ctrlrModulator != nullptr) {
		...
		if (ctrlrComponent != 0) {
			// ctrlrComponent->getOwner().setProperty (Ids::vstIndex,
			// owner.getOwner().getOwner().getVstManager().getFirstFree());
...
CtrlrComponent *CtrlrPanelCanvas::findEventComponent(const MouseEvent &e) {
	...
	} else if (getOwner().getOwner().containsCtrlrComponent(
				   (CtrlrComponent *)e.eventComponent->findParentComponentOfClass<CtrlrComponent>())) {
...
void CtrlrPanelCanvas::dragSelectedComps(int dx, int dy, const bool allowSnap) {
	...
	for (int i = 0; i < getOwner().getSelection()->getNumSelected(); ++i) {
		...
		c->setTopLeftPosition(snapPosition(startX + dx, (int)getOwner().getProperty(Ids::uiPanelSnapSize), allowSnap),
							  snapPosition(startY + dy, (int)getOwner().getProperty(Ids::uiPanelSnapSize), allowSnap));
	}
}
...
void CtrlrPanelCanvas::startDraggingSelection() {
	...
	if (getOwner().getSelection()->getNumSelected() > 1)
		...
		getPanel().getUndoManager()->beginNewTransaction(
			"Move " + getOwner().getSelection()->getSelectedItem(0)->getVisibleName());
}
...
void CtrlrPanelCanvas::restoreState(const ValueTree &savedState) {
	...
	for (int i = 0; i < savedState.getNumChildren(); i++) {
		...
		if (child.hasType(Ids::modulator)) {
			...
			if (c == nullptr) {
				_WRN("CtrlrPanelCanvas::restoreState failed to add component: " +
					 child.getProperty(Ids::name).toString());
			} else {
...
void CtrlrPanelCanvas::editMenuPaste() {
	...
	if (clipboardTree.hasType(Ids::uiPanelClipboardTree)) {
		...
		for (int i = 0; i < clipboardTree.getNumChildren(); i++) {
			...
			_DBG("CtrlrPanelCanvas::editMenuPaste [PRE] vstIndex==" +
				 c->getOwner().getProperty(Ids::vstIndex).toString());
			...
			_DBG("CtrlrPanelCanvas::editMenuPaste [POST] vstIndex==" +
				 c->getOwner().getProperty(Ids::vstIndex).toString());
		}
	}
...
void CtrlrPanelCanvas::moveSelectionBy(const int deltaOffset, const Direction directionToMove) {
	...
	if (directionToMove == Right) {
		for (int i = 0; i < getOwner().getSelection()->getNumSelected(); ++i) {
			CtrlrComponent *const c = getOwner().getSelection()->getSelectedItem(i);
			if (c)
				c->setTopLeftPosition(c->getX() + 1, c->getY());
		}
	}
	...
	if (directionToMove == Down) {
		for (int i = 0; i < getOwner().getSelection()->getNumSelected(); ++i) {
			CtrlrComponent *const c = getOwner().getSelection()->getSelectedItem(i);
			if (c)
				c->setTopLeftPosition(c->getX(), c->getY() + 1);
		}
	}
...
Path CtrlrPopupMenuLook::getTickShape(const float height) {
	static const unsigned char tickShapeData[] = {
		109, 0,	  224, 168, 68,	 0,	  0,   119, 67,	 108, 0,   224, 172, 68,  0,   128, 146, 67,  113, 0,	192, 148,
		68,	 0,	  0,   219, 67,	 0,	  96,  110, 68,	 0,	  224, 56,	68,	 113, 0,   64,	51,	 68,  0,   32,	130, 68,
		0,	 64,  20,  68,	0,	 224, 162, 68,	108, 0,	  128, 3,	68,	 0,	  128, 168, 68,	 113, 0,   128, 221, 67,
		0,	 192, 175, 68,	0,	 0,	  207, 67,	0,	 32,  179, 68,	113, 0,	  0,   201, 67,	 0,	  224, 173, 68,	 0,
		0,	 181, 67,  0,	224, 161, 68,  108, 0,	 128, 168, 67,	0,	 128, 154, 68,	113, 0,	  128, 141, 67,	 0,
		192, 138, 68,  0,	128, 108, 67,  0,	64,	 131, 68,  113, 0,	 0,	  62,  67,	0,	 128, 119, 68,	0,	 0,
		5,	 67,  0,   128, 114, 68,  113, 0,	0,	 102, 67,  0,	192, 88,  68,  0,	128, 155, 67,  0,	192, 88,
		68,	 113, 0,   0,	190, 67,  0,   192, 88,	 68,  0,   128, 232, 67,  0,   224, 131, 68,  108, 0,	128, 246,
		67,	 0,	  192, 139, 68,	 113, 0,   64,	33,	 68,  0,   128, 87,	 68,  0,   0,	93,	 68,  0,   224, 26,	 68,
		113, 0,	  96,  140, 68,	 0,	  128, 188, 67,	 0,	  224, 168, 68,	 0,	  0,   119, 67,	 99,  101};
...
void CtrlrPopupMenuLook::drawPopupMenuBackground(Graphics &g, int width, int height) {
...
void CtrlrPopupMenuLook::drawPopupMenuItem(Graphics &g, int width, int height, const bool isSeparator,
	...
	if (isSeparator) {
		...
		if (image != nullptr) {
			g.drawImageWithin(*image, 2, 1, leftBorder - 4, height - 2,
		...
		g.drawFittedText(ttext, leftBorder, 0, width - (leftBorder + rightBorder), height, Justification::centredLeft,
						 1);
		...
		if (shortcutKeyText.isNotEmpty() || hasSecondLine) {
			...
			g.drawFittedText(textToDraw, secondLineStart + 8 + leftBorder, 0,
							 width - (leftBorder + rightBorder + 4) - secondLineStart - 8, height,
							 Justification::centredRight, 3);
		}
		if (hasSubMenu) {
...
void CtrlrPopupMenuLook::getIdealPopupMenuItemSize(const String &text, bool isSeparator, int standardMenuItemHeight,
...
void CtrlrPanelCanvas::itemDropped(const SourceDetails &dragSourceDetails) {
	...
	if (dragSourceDetails.description == "__ctrlr_component_selection") {
		AffineTransform trans = owner.getOwner().getEditor()->moveSelectionToPosition(
		...
		for (int i = 0; i < owner.getSelection()->getNumSelected(); i++) {
			...
			if ((bool)child->getProperty(Ids::componentGroupped) == true) {
				...
				owner.getSelection()->getSelectedItem(i)->setBounds(
					owner.getSelection()->getSelectedItem(i)->getBounds().transformedBy(trans));
			}
		}
	}
}
...
CtrlrQuickXmlPreview::CtrlrQuickXmlPreview(ValueTree &_treeToPreview)
	: h("Show XML", URL()), treeToPreview(_treeToPreview) {
	addAndMakeVisible(&h);
	h.addListener(this);
	setSize(64, 16);
}
