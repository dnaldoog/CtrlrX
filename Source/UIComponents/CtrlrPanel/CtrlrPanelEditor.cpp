...
void CtrlrPanelNotifier::paint(
    Graphics& g) // Added back v5.6.31 for file management bottom notification bar
{
    gui::drawSelectionRectangle(g, getWidth(), getHeight(),
                                background); // Updated v5.6.31 (link to GUI class)
}
...
void CtrlrPanelNotifier::setNotification(
    ...
    const CtrlrNotificationType
        ctrlrNotificationType) // Added back v5.6.31 for file management bottom notification bar
{
...
CtrlrPanelEditor::CtrlrPanelEditor(CtrlrPanel& _owner, CtrlrManager& _ctrlrManager,
                                   const juce::String& panelName)
    : juce::Component(L"Ctrlr Panel Editor"), lastEditMode(true), ctrlrManager(_ctrlrManager),
      owner(_owner), panelEditorTree(Ids::uiPanelEditor), currentRestoreState(true), canvasWidth(0),
      canvasHeight(0) {
    ...
    ctrlrPanelNotifier->setAlwaysOnTop(
        true); // Added back v5.6.31 for file management bottom notification bar
    ctrlrPanelNotifier->setVisible(
        false); // Added back v5.6.31 for file management bottom notification bar
    ...
    setProperty(Ids::uiPanelBackgroundColour,
                (String)Component::findColour(ResizableWindow::backgroundColourId)
                    .toString()); // Canvas Colour 0xffffffff
    setProperty(Ids::uiPanelBackgroundColour1,
                (String)Component::findColour(ResizableWindow::backgroundColourId)
                    .toString()); // Canvas Colour1 if gradient
    setProperty(Ids::uiPanelBackgroundColour2,
                (String)Component::findColour(ResizableWindow::backgroundColourId)
                    .darker(0.2f)
                    .toString()); // Canvas Colour2 if gradient
    setProperty(Ids::uiPanelBackgroundGradientType,
                0); // Default set to SolidColor [No background gradient]
    ...
    setProperty(Ids::uiPanelTooltipColour,
                (String)Component::findColour(Label::textColourId).toString()); // 0xff000000
    setProperty(Ids::uiPanelTooltipBackgroundColour,
                (String)Component::findColour(BubbleComponent::backgroundColourId)
                    .toString()); // 0xffeeeebb
    setProperty(
        Ids::uiPanelTooltipOutlineColour,
    ...
    if (ed.getProperty(Ids::ctrlrLegacyMode) == "1" ||
        ...
        // Requires passing the colourScheme to the property uiPanelLookAndFeel from
        // ctrlrColourScheme Updated v5.6.34. For a generic method schemeName Property-->
        // schemeName. Get the current colour scheme name from the property
    ...
    //    LookAndFeel_V4::getCurrentColourScheme().getUIColour
    ...
    //    setProperty(Ids::uiPanelUIColourMenuText, (String)
    //    LookAndFeel_V4::getCurrentColourScheme().getUIColour
...
void CtrlrPanelEditor::layoutItems() {
    if (getProperty(Ids::uiPanelEditMode)) {
        ...
        layoutManager.layOutComponents(editorComponents, 1, 0, 0, getWidth(), getHeight(), false,
                                       true);
    }
}
...
void CtrlrPanelEditor::restoreState(const ValueTree& savedState) {
    ...
    bool IsNotLegacyMode =
        savedState.getChildWithName(Ids::uiPanelEditor)
...
void CtrlrPanelEditor::valueTreePropertyChanged(ValueTree& treeWhosePropertyHasChanged,
    ...
    if (treeWhosePropertyHasChanged.hasType(Ids::uiPanelEditor)) {
        if (property == Ids::uiPanelEditMode) {
            ...
            resizedCbk =
                owner.getCtrlrLuaManager().getMethodManager().getMethod(getProperty(property));
            ...
            getCanvas()->setBounds(VAR2RECT(getProperty(
                property))); // update canvas size if values in the property field are changed
            canvasHeight = getCanvas()->getHeight(); // Updated v5.6.31 by GoodWeather. Removed type
                                                     // double(canvasHeight)
            canvasWidth = getCanvas()->getWidth();   // Updated v5.6.31 by GoodWeather. Removed type
                                                     // double(canvasWidth)
            canvasAspectRatio =
                canvasWidth / canvasHeight; // Updated v5.6.31 by GoodWeather. Removed type
                                            // double(canvasAspectRatio) = double(canvasWidth) /
                                            // double(canvasHeight)
            ...
            resized();
                   ...
                   property == Ids::uiViewPortMinWidth || property == Ids::uiViewPortMinHeight ||
            ...
            auto newLookAndFeel = std::unique_ptr<juce::LookAndFeel>(
                getLookAndFeelFromDescription(getProperty(property)));

            if (newLookAndFeel == nullptr)
                return;

            // 1. Clear out active component styling links FIRST
            getCanvas()->setLookAndFeel(nullptr);
            setLookAndFeel(nullptr);

            // 2. Transfer ownership to the class member.
            //    This cleanly destroys the PREVIOUS LookAndFeel object now.
            lookAndFeel = std::move(newLookAndFeel);

            // 3. Apply the NEW LookAndFeel address from our permanent class container
            getCanvas()->setLookAndFeel(lookAndFeel.get());
            setLookAndFeel(lookAndFeel.get());
            LookAndFeel::setDefaultLookAndFeel(lookAndFeel.get());

            // 4. CRITICAL: Tell every child UI component to flush its cache and redraw!
            lookAndFeelChanged();

            if (!getProperty(Ids::uiPanelLegacyMode)) // Added v5.6.30. Protects Legacy panels' BKG
                                                      // Colours when being assigned LnF V3
            {
                // Update colors based on the new valid LookAndFeel.
                setProperty(Ids::uiPanelViewPortBackgroundColour,
                            (String)Component::findColour(ResizableWindow::backgroundColourId)
                                .withAlpha(0.7f)
                                .toString()); // Update Canvas props
                setProperty(
                ...
                setProperty(Ids::uiPanelBackgroundColour2,
                            (String)Component::findColour(ResizableWindow::backgroundColourId)
                                ...
                                .toString());
                setProperty(
                    Ids::uiPanelTooltipBackgroundColour,
                ...
                setProperty(Ids::uiPanelTooltipColour,
                            (String)Component::findColour(Label::textColourId).toString());
            }
            ...
            if (getSelection()) {
                getSelection()
                    ->sendChangeMessage(); // Brings back the screen position to where it left
            }
            ...
            //            auto* customLookAndFeel =
            //            dynamic_cast<LookAndFeel_V4*>(&getLookAndFeel()); auto
            //            customLookAndFeelScheme = customLookAndFeel->getCurrentColourScheme();
            //
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::windowBackground,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourWindowBackground)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::widgetBackground,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourWidgetBackground)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::menuBackground,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourMenuBackground)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::outline,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourOutline)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::defaultText,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourDefaultText)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::defaultFill,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourDefaultFill)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::highlightedText,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourHighlightedText)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::highlightedFill,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourHighlightedFill)));
            //            getCurrentColourScheme().setUIColour(ColourScheme::UIColour::menuText,
            //            VAR2COLOUR (owner.getProperty(Ids::uiPanelUIColourMenuText)));
            //
...
juce::LookAndFeel* CtrlrPanelEditor::getLookAndFeelFromDescription(
    const juce::String& lookAndFeelDesc) // Added v5.6.34
{
...
const var CtrlrPanelEditor::getProperty(const Identifier& name,
                                        const var& defaultReturnValue) const {
    return (panelEditorTree.getProperty(name, defaultReturnValue));
}

void CtrlrPanelEditor::setProperty(const Identifier& name, const var& newValue,
        ...
        ctrlrNotificationType) // Added back v5.6.31 for file management bottom notification bar
{
    if (ctrlrPanelNotifier) {
        ...
        componentAnimator.animateComponent(ctrlrPanelNotifier, ctrlrPanelNotifier->getBounds(),
                                           0.0f, owner.getProperty(Ids::panelMessageTime), false,
                                           1.0, 1.0);
    }
}
...
bool CtrlrPanelEditor::luaEditorExistsAndIsFocused() // Added v5.6.34. Required to pass keypress to
                                                     // the LUA method manager for menu items.
                                                     // Handles the focus gain/loss.
{
