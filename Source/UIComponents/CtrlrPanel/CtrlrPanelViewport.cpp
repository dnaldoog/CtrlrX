...
void CtrlrPanelViewport::resized() { viewport->setBounds(0, 0, getWidth(), getHeight()); }
...
void CtrlrPanelViewport::setZoom(const double zoom, int anchorX, int anchorY) {
	...
	viewport->setViewPosition(jlimit(0, jmax(0, viewport->getViewedComponent()->getWidth() - viewport->getViewWidth()),
							  ...
							  jlimit(0,
									 jmax(0, viewport->getViewedComponent()->getHeight() - viewport->getViewHeight()),
									 viewport->getViewPositionY() + anchor.getY() - anchorY));
	magnifier->setScaleFactor(currentZoom);
}

const double CtrlrPanelViewport::getZoom() { return (currentZoom); }
...
bool CtrlrViewportImpl::keyPressed(const KeyPress &key, Component *originatingComponent) { return (false); }

bool CtrlrViewportImpl::keyStateChanged(bool isKeyDown, Component *originatingComponent) { return (false); }
...
const int CtrlrPanelViewport::getNumCanvases() { return (canvasList.size()); }

void CtrlrPanelViewport::moveCanvasUp(const int canvasIndex) {}

void CtrlrPanelViewport::moveCanvasDown(const int canvasIndex) {}

CtrlrPanelCanvas *CtrlrPanelViewport::getCurrentCanvas() { return (canvasList[0]); }

void CtrlrPanelViewport::lookAndFeelChanged() { resized(); }
...
const var &CtrlrPanelViewport::getProperty(const Identifier &name) const { return (owner.getProperty(name)); }
...
ValueTree &CtrlrPanelViewport::getViewPortTree() { return (owner.getPanelEditorTree()); }

CtrlrPanelCanvas *CtrlrPanelViewport::getCanvas(const int canvasIndex) {
	if (canvasIndex < 0 || canvasIndex >= canvasList.size())
		return nullptr;

	return canvasList[canvasIndex];
}