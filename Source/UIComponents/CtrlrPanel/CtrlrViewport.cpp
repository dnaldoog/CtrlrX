...
CtrlrViewport::~CtrlrViewport() { deleteContentComp(); }
...
void CtrlrViewport::setViewedComponent(Component *const newViewedComponent,
									   const bool deleteComponentWhenNoLongerNeeded) {
...
void CtrlrViewport::setViewPosition(const int xPixelsOffset, const int yPixelsOffset) {
	if (contentComp != nullptr)
		contentComp->setTopLeftPosition(
			jmax(jmin(0, contentHolder.getWidth() - contentComp->getWidth()), jmin(0, -xPixelsOffset)),
...
bool CtrlrViewport::autoScroll(const int mouseX, const int mouseY, const int activeBorderThickness,
	...
	if (contentComp != nullptr) {
		...
		if (dx != 0 || dy != 0) {
			contentComp->setTopLeftPosition(contentComp->getX() + dx, contentComp->getY() + dy);
...
void CtrlrViewport::componentMovedOrResized(Component &, bool, bool) { updateVisibleArea(); }

void CtrlrViewport::resized() { updateVisibleArea(); }
...
void CtrlrViewport::updateVisibleArea() {
	...
	if (contentComp != nullptr && !contentArea.contains(contentComp->getBounds())) {
		hBarVisible =
			canShowHBar && (hBarVisible || contentComp->getX() < 0 || contentComp->getRight() > contentArea.getWidth());
		vBarVisible = canShowVBar &&
					  (vBarVisible || contentComp->getY() < 0 || contentComp->getBottom() > contentArea.getHeight());
...
void CtrlrViewport::setScrollBarsShown(const bool showVerticalScrollbarIfNeeded,
	...
	if (showVScrollbar != showVerticalScrollbarIfNeeded || showHScrollbar != showHorizontalScrollbarIfNeeded) {
...
int CtrlrViewport::getScrollBarThickness() const {
	return scrollBarThickness > 0 ? scrollBarThickness : getLookAndFeel().getDefaultScrollbarWidth();
}
	...
	if (!(e.mods.isAltDown() || e.mods.isCtrlDown())) {
		...
		if (hasHorzBar || hasVertBar) {
			if (wheelIncrementX != 0) {
				...
				wheelIncrementX = (wheelIncrementX < 0) ? jmin(wheelIncrementX, -1.0f) : jmax(wheelIncrementX, 1.0f);
			}

			if (wheelIncrementY != 0) {
				wheelIncrementY *= 14.0f * singleStepY;
				wheelIncrementY = (wheelIncrementY < 0) ? jmin(wheelIncrementY, -1.0f) : jmax(wheelIncrementY, 1.0f);
			}
