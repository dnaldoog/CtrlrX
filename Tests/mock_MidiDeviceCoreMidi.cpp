...
// Somewhere in JUCE 8.0.x the universal_midi_packets conversion headers moved from
// juce_audio_devices/midi_io/ump/ to juce_audio_basics/midi/ump/, and juce_audio_basics.h now
// includes them unconditionally.
// On an older JUCE (e.g. the master) they are pulled in only
// inside juce_audio_devices.cpp and are invisible through <JuceHeader.h>.
// In that case the (header-only) converter chain has to be included by hand
//
// JUCE defines no feature macro for the move, so probe for the new umbrella header instead of
// testing JUCE_VERSION against a cutoff we would have to guess.
#if __has_include(<juce_audio_basics/midi/ump/juce_UMP.h>)
#define CTRLRX_UMP_IN_AUDIO_BASICS 1
#else
#define CTRLRX_UMP_IN_AUDIO_BASICS 0
...
// ump::Iterator's second ctor argument is the size of the range it may walk. Old JUCE measured
// it in bytes, new JUCE in words. It only feeds a JUCE_DEBUG bounds jassert, but getting it
// wrong makes that assert four times too lax.
constexpr size_t kUmpIteratorUnit = CTRLRX_UMP_IN_AUDIO_BASICS ? 1 : sizeof(juce::uint32);
...
int mockNumEndpoints() {
	return mockNumDevices() * mockNumPorts();
}
...
bool decodeEndpoint(MIDIObjectRef ref, int &dev, int &port, bool &isSource) {
	...
	int flat;
	if (ref >= kSourceBase && ref < kSourceBase + kRangeSize) {
		flat = (int)(ref - kSourceBase);
		isSource = true;
	} else if (ref >= kDestBase && ref < kDestBase + kRangeSize) {
		flat = (int)(ref - kDestBase);
		isSource = false;
	} else
		return false;

	if (flat >= mockNumEndpoints())
		return false;
	dev = flat / np;
	port = flat % np;
	return true;
...
void deliverToBlocks(const std::vector<MIDIReceiveBlock> &blocks, const juce::MidiMessage &msg) {
	...
	const auto appendPacket = [&](const ump::View &view) {
		packet = MIDIEventListAdd(list, sizeof(storage), packet, /*timeStamp*/ 0, view.size(), view.data());
	};

#if CTRLRX_UMP_IN_AUDIO_BASICS
	// New JUCE takes the bytestream as a BytesOnGroup (group 0 == the only group we use);
	// the MidiMessage overload is gone.
	converter.convert(ump::BytesOnGroup{0, juce::Span{reinterpret_cast<const std::byte *>(msg.getRawData()),
													  (size_t)msg.getRawDataSize()}},
					  appendPacket);
#else
	converter.convert(msg, appendPacket);
#endif
...
ItemCount MIDIGetNumberOfSources(void) {
	return (ItemCount)mockNumEndpoints();
}
...
OSStatus MIDIEndpointGetEntity(MIDIEndpointRef, MIDIEntityRef *outEntity) {
	if (outEntity != nullptr)
		*outEntity = 0; // 0 -> JUCE takes the "virtual endpoint" branch
	return noErr;
}
...
OSStatus MIDIInputPortCreateWithProtocol(MIDIClientRef, CFStringRef, MIDIProtocolID, MIDIPortRef *outPort,
...
OSStatus MIDISendEventList(MIDIPortRef, MIDIEndpointRef dest, const MIDIEventList *evtList) {
	...
	for (UInt32 i = 0; i < evtList->numPackets; ++i) {
		ump::Iterator it(packet->words, packet->wordCount * kUmpIteratorUnit);
		ump::Iterator end(packet->words + packet->wordCount, 0);
		for (; it != end; ++it)
#if CTRLRX_UMP_IN_AUDIO_BASICS
			// New JUCE hands the reassembled bytestream to the callback as (BytesOnGroup, time)
			// rather than as a ready-made MidiMessage.
			converter.convert(*it, 0.0, [&](const ump::BytesOnGroup &bytes, double time) {
				m->sendMidiEvent(dev, port, juce::MidiMessage(bytes.bytes.data(), (int)bytes.bytes.size(), time));
			});
#else
			converter.convert(*it, 0.0, [&](const juce::MidiMessage &msg) { m->sendMidiEvent(dev, port, msg); });
#endif
...
OSStatus MIDIDestinationCreateWithProtocol(MIDIClientRef, CFStringRef, MIDIProtocolID, MIDIEndpointRef *outDest,
										   MIDIReceiveBlock) {
	if (outDest != nullptr)
		*outDest = kDestBase; // arbitrary non-zero
	return noErr;
}
OSStatus MIDISourceCreateWithProtocol(MIDIClientRef, CFStringRef, MIDIProtocolID, MIDIEndpointRef *outSrc) {
	if (outSrc != nullptr)
		*outSrc = kSourceBase;
	return noErr;
}
